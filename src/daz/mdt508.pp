...
\def\Title{Module Tests for Basic Declarations and Expressions}

\def\AbstractText{This document comprises test cases and test code/data for testing the Declaration and Expression generator as described in the document ISS/HAT/DAZ/DTD508.}

\def\Reference{ISS/HAT/DAZ/MDT508}
...
%% LaTeX2e port: \TPPtitle{Module Tests for Basic Declarations and Expressions}  %% Mandatory field
%% LaTeX2e port: \TPPref{ISS/HAT/DAZ/MDT508}  %% Mandatory field
%% LaTeX2e port: \def\SCCSversion{$Revision: 1.45 $
%% LaTeX2e port: }
%% LaTeX2e port: \TPPissue{\SCCSversion}  %% Mandatory field
%% LaTeX2e port: \TPPdate{\FormatDate{$Date: 2006/06/12 16:42:15 $
...
%% LaTeX2e port: testing the Declaration and Expression generator as described in the document
%% LaTeX2e port: ISS/HAT/DAZ/DTD508.}
...
store_mt_results (mt_runf comp_z_exp) [
    ...
    ("zabstract.35", (fn x => x),
        z_set_comp (mk_z_dec ([ⓩa⌝],ⓩℤ⌝),ⓩa>42⌝), "ⓩ{a : ℤ | a > 42}⌝"),
...
store_mt_results (mt_runf comp_z_exp) [
    ...
    ("zabstract.56", (fn x => x),
        z_forall ([mk_z_dec ([ⓩa⌝],ⓩℤ⌝)], ⓩtrue⌝), "ⓩ∀ a : ℤ ⦁ true⌝"),
...
store_mt_results (mt_runf comp_abbdef_info) [
    ("zabstract.59", (fn x => x), z_eq_eq "test" ("TODAY", ⓩtrue⌝), AbbDefInfo
      ("TODAY", [], ⓩtrue⌝)),
    ("zabstract.60", (fn x => x), z_schema ("TODAY", [mk_z_dec ([ⓩa⦂ℤ⌝],ⓩℤ⌝)], z_true),
      AbbDefInfo ("TODAY", [], ⓩ[a : ℤ]⌝))];
=TEX
...
val _ = store_mt_results (mt_runf (op =$)) [
    ...
    ("exp_unary_exp.6", exptest, "not flags1", ⓩarray_not (FLAGS1⦂(ℤ ⇸ ℤ))⌝)];
=TEX
=SML
val _ = store_mt_results (mt_runf (op =$)) [
...
val _ = store_mt_results (mt_runf (op =$)) [
    ...
    ("exp_binary_exp.7", exptest, "a = b", ⓩ(A⦂ℤ) eq B⌝),
    ("exp_binary_exp.8", exptest, "a /= b", ⓩ(A⦂ℤ) noteq B⌝),
    ...
    ("exp_binary_exp.17", exptest, "flags1 and flags2", ⓩ(FLAGS1⦂(ℤ ⇸ ℤ)) array_and FLAGS2⌝),
    ("exp_binary_exp.18", exptest, "flags1 or flags2", ⓩ(FLAGS1⦂(ℤ ⇸ ℤ)) array_or FLAGS2⌝),
    ("exp_binary_exp.19", exptest, "flags1 xor flags2", ⓩ(FLAGS1⦂(ℤ ⇸ ℤ)) array_xor FLAGS2⌝),
    ("exp_binary_exp.20", exptest, "flags1 > flags2", ⓩ(FLAGS1⦂(ℤ ⇸ ℤ)) array_greater FLAGS2⌝),
    ("exp_binary_exp.21", exptest, "flags1 < flags2", ⓩ(FLAGS1⦂(ℤ ⇸ ℤ)) array_less FLAGS2⌝),
    ("exp_binary_exp.22", exptest, "flags1 >= flags2", ⓩ(FLAGS1⦂(ℤ ⇸ ℤ)) array_greater_eq FLAGS2⌝),
    ("exp_binary_exp.23", exptest, "flags1 <= flags2", ⓩ(FLAGS1⦂(ℤ ⇸ ℤ)) array_less_eq FLAGS2⌝)];
=TEX
=SML
val _ = store_mt_results (mt_runf (op =$)) [
...
=SML
local
val _ = change_env "DAYRANGE" {tipe = Value (type_ofⓩℤ⌝), canon_name = "DAYRANGE", info = EIEnvOtherType, private = false};
val arg = "DAYRANGE(11)";
val res = ⓩ11⌝;
in
val _ = store_mt_results (mt_runf (op =$))
...
=SML
val _ = store_mt_results (mt_runf (op =)) [
    ("record", bdtest, ⓩμ DATE | DAY=6 ∧ MONTH=8 ⦁ INTEGER⌝, true)];
=TEX
...
val _ = store_mt_results (mt_runf (op =)) [
    ...
    ("integer.3", bdtest, ⓩHOURSvSUCC ∪ HOURSvPRED ⊂ ℤ × ℤ⌝, true),
    ("integer.4", bdtest, ⓩHOURSvPOS ∪ HOURSvVAL ⊂ ℤ × ℤ⌝, true)
];
=TEX
\subsection{Real Types}
=SML
val _ = store_mt_results (mt_runf (op =)) [
    ("real.1", bdtest, ⓩFIX ∈ ℙ FIX⌝, true),
    ("real.2", bdtest, ⓩFLOAT1 ∈ ℙ FLOAT1⌝, true)
];
=TEX
...
val _ = store_mt_results (mt_runf (op =)) [
    ...
    ("subtype.3", bdtest, ⓩWEEKDAYvSUCC ∪ WEEKDAYvPRED ⊆ ℤ × ℤ⌝, true),
    ("subtype.4", bdtest, ⓩWEEKDAYvPOS ∪ WEEKDAYvVAL ⊆ ℤ × ℤ⌝, true)
];
=TEX
=SML
val _ = store_mt_results (mt_runf (op =)) [
