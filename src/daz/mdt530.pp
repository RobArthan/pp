=TEX
%   ℤ $Date: 2008/12/11 17:57:34 $ $Revision: 1.2 $ $RCSfile: mdt530.doc,v $
=TEX
...
│ ⦏mean⦎ : ℝ × ℝ → ℝ
├──────
│ ∀x, y:ℝ⦁ mean(x, y) = (x +⋎R y) /⋎R 2.0
■
=SML
push_consistency_goal ⓩmean⌝;
a(∃_tac⌜ⓩλxy:𝕌 × 𝕌⦁ (xy.1 +⋎R xy.2) /⋎R 2.0⌝⌝);
a(rewrite_tac[z_get_specⓩℝ⌝] THEN REPEAT strip_tac);
(* *** Goal "1" *** *)
...
│ ⦏sum_squares⦎ : ℝ × ℝ → ℝ
├──────
│ ∀x, y:ℝ⦁ sum_squares(x, y) = x ^⋎Z 2 +⋎R y ^⋎Z 2
■
=TEX
=SML
