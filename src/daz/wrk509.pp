...
********************************************************************************
% ℤ $Date: 2002/10/17 16:04:45 $ $Revision: 1.14 $ $RCSfile: wrk509.doc,v $
=TEX
...
However, any expression that is meant to be statically evaluated to a $ℤ$ value
...
global definition of no further use.
Some rewriting use may be made of the fact that $succ$ is already domain restricted to $ℕ$.
...
Any of these would make the original
global definition of no further use.
Some rewriting use may be made of the fact that the range of $succ$ is restricted to $ℕ⋎1$.
...
\Product\ has a standard approach to this so-called 𝕌-elimination.
...
There are various possibilities for subtypes, depending on whether or not they are subtypes of types that are translated into $ℤ$, and on the nature of their constraint (e.g. can it be translated into a range constraint).

\subsubsection{Case 1}
If the master type, $master$, can be translated to $ℤ$ and the constraint can be translated to a $ℤ$ range constraint $exp⋎1 .. exp⋎2$ global definitions are introduced for the type and its attributes:
ⓈZ
name == exp⋎1 .. exp⋎2
■
...
When either the constraint is not a range constraint that can be translated into $ℤ$, or the master type is not $ℤ$, or any other case not covered in the above, then
...
\begin{enumerate}
\item
build only from items that concern items of type $ℤ$ or $ℙ ℤ$;
\item
