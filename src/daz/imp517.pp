...
val local_id_sig_thm = tac_proof(
	([],  ⌜∀ X f ⦁
	  ...
	  [ all_var_elim_asm_tac,
	    ...
	    z_∃_tac ⓩx1⌝ THEN PC_T "z_rel_ext" (REPEAT z_strip_tac)
	  ])
	);
=TEX
=SML
...
=TEX
=SML
val local_◁_succ_thm = tac_proof(
	([],  ⌜∀ f j k ⦁ ⓩ 0 ≤ j ⇒
			(f = (j .. k) ◁ succ) ⇒
			(∀ i : j .. k ⦁ f i = i + 1)⌝⌝),
	  PC_T "z_library1"
	  ...
	  z_app_eq_tac THEN
	  rewrite_tac[z_◁_thm, z_∈_succ_thm] THEN
	  REPEAT strip_tac THEN
	  PC_T1 "z_lin_arith1" asm_prove_tac[]
	  ));
=TEX
=SML
val local_◁_succ_sig_thm = tac_proof(
	([],  ⌜∀ f j k ⦁ ⓩ 0 ≤ j ⇒
			(f = (j .. k) ◁ succ) ⇒
			(f ∈ (j .. k) → (j + 1 .. k + 1))⌝⌝),
	  PC_T "z_library1"
	  ...
	  [(DROP_NTH_ASM_T 1 (rewrite_thm_tac o rewrite_rule[z_∈_succ_thm]) THEN
		DROP_NTH_ASM_T 1 (rewrite_thm_tac o rewrite_rule[])
		),
	   (DROP_NTH_ASM_T 1 (rewrite_thm_tac o rewrite_rule[z_∈_succ_thm]) THEN
		DROP_NTH_ASM_T 1 (rewrite_thm_tac o rewrite_rule[z_∈_succ_thm])),
	   (z_∃_tac ⓩx1 + 1⌝ THEN PC_T "z_rel_ext" (REPEAT z_strip_tac) THEN
		rewrite_tac[z_∈_succ_thm] THEN
		DROP_NTH_ASM_T 1 (strip_asm_tac o rewrite_rule[]) THEN
		all_asm_fc_tac[z_≤_trans_thm])	
	  ]
	  ));
=TEX
=SML
val local_◁_succ_rel_inv_thm = tac_proof(
	([],  ⌜∀ f j k ⦁ ⓩ 0 ≤ j ⇒
			(f = ((j .. k) ◁ succ) ↗~↕) ⇒
			(∀ i : j + 1 .. k + 1⦁ f i = i + ~ 1)⌝⌝),
	  PC_T "z_library1"
	  ...
	  DROP_NTH_ASM_T 3 rewrite_thm_tac THEN
	  z_app_eq_tac THEN
	  rewrite_tac[z_rel_inv_thm, z_◁_thm, z_∈_succ_thm] THEN
...
=TEX
=SML
val local_◁_succ_rel_inv_sig_thm = (
	push_pc "z_library1";
	set_goal([],  ⌜∀ f j k ⦁ ⓩ 0 ≤ j ⇒
			(f = ((j .. k) ◁ succ) ↗~↕) ⇒
			(f ∈ j + 1  .. k + 1 → (j .. k))⌝⌝);
	...
	(* *** Goal "1" *** *)
	a(DROP_NTH_ASM_T 1 (strip_asm_tac o rewrite_rule[z_∈_succ_thm]) THEN
		all_var_elim_asm_tac1 THEN
		rewrite_tac[] THEN
		DROP_NTH_ASM_T 2 (rewrite_thm_tac o rewrite_rule[]));
	(* *** Goal "2" *** *)
	a(DROP_NTH_ASM_T 1 (strip_asm_tac o rewrite_rule[z_∈_succ_thm]) THEN
		DROP_NTH_ASM_T 3 (strip_asm_tac o rewrite_rule[z_∈_succ_thm]) THEN
		all_var_elim_asm_tac1 THEN
		DROP_NTH_ASM_T 1 (rewrite_thm_tac o rewrite_rule[]));
	(* *** Goal "4" *** *)
	a(z_∃_tac ⓩx1 + (~ 1)⌝ THEN rewrite_tac[z_rel_inv_thm, z_◁_thm, z_∈_succ_thm]	THEN
		DROP_NTH_ASM_T 1 (strip_asm_tac o rewrite_rule[]) THEN
		PC_T1 "z_lin_arith1" asm_prove_tac []);
	pop_pc();
	pop_thm ()
...
val local_set_abs_thm = (
	  set_goal([],  ⌜∀ f master index X Y ⦁ ⓩ
		...
		a(strip_tac THEN z_∀_tac THEN strip_tac);
		a(POP_ASM_T (strip_asm_tac o z_∀_elim ⓩx1⌝));
(* *** Goal "1" *** *)
		a(asm_rewrite_tac[]);
(* *** Goal "2" *** *)
		a(cases_tac ⓩx1 ∈ master⌝ THEN asm_rewrite_tac[]);
...
(* *** Goal "2.2" *** *)
		a(asm_fc_tac[]);
(* *** Goal "2.3" *** *)
		a(z_spec_nth_asm_tac 7 ⓩ(x ≜ x, y1 ≜ y1, y2 ≜ y2)⌝);
		a(z_spec_nth_asm_tac 9 ⓩ(x2 ≜ x, x3 ≜ y1)⌝);
		pop_pc();
		pop_thm()
	  );
=TEX
...
in
	...
	tac_proof(([], mk_z_eq (tm, rhs)),
		PC_T "z_library1" (
			conv_tac z_sets_ext_conv THEN z_∀_tac THEN z_strip_tac THEN
			conv_tac (ONCE_MAP_C z_∈_h_schema_conv) THEN
			TRY_T(conv_tac (MAP_C z_setd_⊆_conv)) THEN
			taut_tac
		)
	)
end
...
=SML
	...
	val pat1 = ⓩid ⌜pp'name⌝⌝;
	val pat2 = ⓩ⌜pp'name⌝ ↗~↕⌝;
	val pat3 = ⓩ ( ⌜pp'name⌝ \ { ⌜pp'lst⌝ } ) ◁ succ⌝;
	val pat4= ⓩ {u : ⌜pp'master⌝ | dom u = ⌜pp'index⌝ } ⌝;
	val pat5 = ⓩ⌜pp'lhs⌝ = ( ⌜pp'first⌝ .. ⌜pp'lstm1⌝  ) ◁ succ⌝;
	val pat6 = ⓩ⌜pp'lhs⌝ = (( ⌜pp'first⌝ .. ⌜pp'lstm1⌝  ) ◁ succ) ↗~↕⌝;
	val pat7 = ⓩ⌜pp'master⌝ ∈ ℙ ( ⌜pp'X⌝ ⇸ ⌜pp'Y⌝)⌝;

	val local_rw_rule = pure_rewrite_rule[z_plus_assoc_thm,
		∧_right_elim(z_get_spec ⓩ(_ - _)⌝), z_minus_clauses,
		z_plus0_thm];
...
=SML	
	...
	(let 	val (lhs, rhs) = dest_z_eq (concl thm);
		...
		val thm1 = list_∀_elim [name, lhs] local_id_thm;
		val thm2 = ⇒_elim thm1 thm;
		val thm3 = list_∀_elim [name, lhs] local_id_sig_thm;
...
=TEX
=IGN
pat3_canon (z_get_spec ⓩDAYvSUCC⌝);
=SML
	fun pat3_canon thm =
	(let 	val (lhs, rhs) = dest_z_eq (concl thm);
		...
		val first2 = snd(dest_eq (concl first_spec));
 		val thm4 = ⇔_t_elim (z_≤_conv (mk_z_≤ (nat0, first2)));
		val thm5 = conv_rule (LEFT_C (RIGHT_C
			(eq_match_conv (eq_sym_rule first_spec)))) thm4;
		val last2 = mk_z_plus(last_rhs, ⓩ~ 1⌝);
		val thm6 = list_∀_elim [lhs, first, last2]
			local_◁_succ_thm;
		val thm7 = ⇒_elim (⇒_elim thm6 thm5) thm3;
		val thm8 = list_∀_elim [lhs, first, last2] local_◁_succ_sig_thm;
...
=IGN
pat2_canon (z_get_spec ⓩDAYvPRED⌝);
=SML
	fun pat2_canon thm =
	(let 	val (lhs, rhs) = dest_z_eq (concl thm);
	...
	in
		(let	val name2 = match1 nm1_rhs pat1;
			...
			val thm3 = list_∀_elim [name2, lhs] local_id_thm;
			val thm4 = ⇒_elim thm3 thm2;
			val thm5 = list_∀_elim [name2, lhs] local_id_sig_thm;
		...
		(let	val (last, nm2) = match2 nm1_rhs pat3;
			...
			val last2 = mk_z_plus(last_rhs, ⓩ~ 1⌝);
			val thm7 = list_∀_elim [lhs, first, last2]
				local_◁_succ_rel_inv_thm;
			val thm8 = ⇒_elim (⇒_elim thm7 thm6) thm4;
			val thm9 = local_rw_rule thm8;
			val thm10 = list_∀_elim [lhs, first, last2]
				local_◁_succ_rel_inv_sig_thm;
...
=SML
	...
	(let 	val (lhs, rhs) = dest_z_eq (concl thm);
		...
		val thm1 = list_∀_elim[lhs, master, index, X, Y]
			 local_set_abs_thm;
...
fun ⦏strip_decls_canon⦎ (thm : THM) : THM list = (
	...
	else ((let val thm1 = (conv_rule z_setd_⊆_conv thm)
		in
			strip_decls_canon thm1
		end)
		handle (Fail _) =>
		(if is_z_∈ (concl thm)
		then (let val (a, _) =  dest_z_∈ (concl thm)
			in
...
=IGN
strip_decls_canon (∧_left_elim (z_get_spec ⓩ(not_)⌝));
=TEX
=SML
...
cn_spec_rule (get_spec ⓩ(not_)⌝);
cn_spec_rule(get_spec ⓩ(bag_)⌝);
=SML
local val tru = ⓩtrue⌝
in
...
in
	...
	then (let val (full_nm, _, _) = dest_z_gvar(fst(dest_eq cthm)) ;
		...
		(fst (dest_app cthm) =$ ⌜$"Z'Constraint"⌝)
...
(let	val thm0 = conv_rule z_para_pred_conv thm
	...
	val thm1 = all_∀_elim thm0;
in
	map (fn (n, t) => (n, all_∀_intro t)) (* reintroduce variables *)
...
let 	val dummy_pc1 = push_pc "cn1";
	...
	val thms = filter (fn x =>
		((z_∈_u_conv (concl x); false) handle (Fail _) => true))
		sort_thms;
	val u_thms = filter is_U_eqn thms;
	val not_u_thms = filter (fn x => not(is_U_eqn x)) thms;
	val source_ec = add_∈(thms_to_eqn_cxt not_u_thms);
	...
	val dummy8 = set_pc_z_∈_rules
		(pc_z_∈_rules_of_thms [pcname] not_u_thms)
		pcname;
...
val nets = get_pc_z_∈_rules "cn_pc";
