...
********************************************************************************
% ℤ $Revision: 1.21 $ $RCSfile: int507.doc,v $ $Date: 2006/09/16 14:05:32 $
...
begin
   A := 7;
   B := 1;
   Δ A, B [A = A⋎0 + 2 ∧ B = B⋎0 + 10]				(1)
end P;
■

ⓈCN
(1) ⊑ $con X : ℤ ⦁ Δ A, B [X = A, A = X + 2 ∧ B = B⋎0 + 10]	(2)
■
=SML
fun var_capt_3 () =
ⓈCN
(2) ⊑ $con X : ℤ ⦁ Δ A, B [X = B, A = X + 2 ∧ B = X + 10]	(3)
■
=GFT Compliance Notation Example
(3) ⊑
      A := B + 2;
...
package STACK is
   ...
   Δ S, MAX [MAX = MAX⋎0 + 1 ∧ S = S⋎0 ⊕ {MAX ↦ X}];
end STACK;
■
...
is
 procedure Q Δ[true];
 function R return INTEGER Ξ[true];
 ⟨ DECS ⟩			( 2 )
