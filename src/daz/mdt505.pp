...
val vct2 = vc "Δ A [A=0, true]";
val arg2 = "NULL;";
val res2 = [([ⓩⓜdec_ofⓩ[A : INTEGER]⌝⌝⌝],([ⓩA=0⌝],ⓩtrue⌝))] : VC list;

val vct3 = vc "Δ A [A=0, A=0]";
val arg3 = "NULL;";
...
val vct1 = vc "Δ HOURS, MON [HOURS(MON) = 7]";
val arg1 = "hours(mon) := 7 ;";
val res1 = [([ⓜdec_ofⓩ[HOURS : ARR]⌝⌝],([ⓩtrue⌝],ⓩ(HOURS ⊕ {MON ↦ 7}) MON = 7⌝))] : VC list;

val vct2 = vc "Δ HOURS, SUN, MON [HOURS(MON) > HOURS(SUN)]";
...
val _ = change_env "COMPLEX"
		{
			...
			info = EIEnvRecordType  {
				...
				num_discs = 0 },
			private = false  } ;
val vct1 = vc "Δ COMP [(COMP⦂COMPLEX).R = 1]";
val arg1 = "comp.r := 1 ;";
...
val vct1 = vc "Δ A [A ≥ 0]";
val arg1 = "Δ A [A ∈ 128..255] (100)";
val res1 = [
	([],([ⓩtrue⌝], ⓩtrue⌝)),
	([ⓜdec_ofⓩ[A : INTEGER]⌝⌝],([ⓩtrue⌝, ⓩA ∈ 128 .. 255⌝], ⓩA ≥ 0⌝))
] : VC list;

val vct2 = vc "Δ A [A = 0, A = A⋎0 + 1]";
val arg2 = "Δ A [A = 1] (100)";
val res2 = [
...
val vct1 = vc "Δ A, B [A = 1, B = 0]";
val arg1 = "a := 0; b := a;";
val res1 = [([ⓜdec_ofⓩ[A : INTEGER]⌝⌝],([ⓩA = 1⌝], ⓩ0 = 0⌝))] : VC list;

val vct2 = vc "Δ A, B [A = 0, A = A⋎0 + 1]";
...
val vct1 = vc "Δ A, B, C, D, SUN [A = 0 ∧ B = 0 ∧ C = 1, D = SUN]";
val arg1 = "\
\       CASE a IS \
\               WHEN b =>       d := sun; \
\               WHEN c =>       d := mon; \
...
\";
val res1 = [
	([ⓜdec_ofⓩ[A, B, C : INTEGER]⌝⌝], ([ⓩA = 0 ∧ B = 0 ∧ C = 1⌝, ⓩA ∈ {B⦂ℤ}⌝], ⓩSUN = SUN⌝)),
	([ⓜdec_ofⓩ[A, B, C : INTEGER]⌝⌝], ([ⓩA = 0 ∧ B = 0 ∧ C = 1⌝, ⓩA ∈ {C⦂ℤ}⌝], ⓩMON = SUN⌝))
] : VC list;
...
val vct2 = vc "Δ A, B, C, D, SUN [A = 0 ∧ B = 0 ∧ C = 1, D = SUN]";
val arg2 = "\
\       CASE a IS \
\               WHEN b =>       d := sun; \
\               WHEN c =>       d := mon; \
...
val res2 = [
	([ⓜdec_ofⓩ[A, B, C : INTEGER]⌝⌝], ([ⓩA = 0 ∧ B = 0 ∧ C = 1⌝, ⓩA ∈ {B⦂ℤ}⌝], ⓩSUN = SUN⌝)),
	([ⓜdec_ofⓩ[A, B, C : INTEGER]⌝⌝], ([ⓩA = 0 ∧ B = 0 ∧ C = 1⌝, ⓩA ∈ {C⦂ℤ}⌝], ⓩMON = SUN⌝)),
	([ⓜdec_ofⓩ[A, B, C : INTEGER]⌝⌝], ([ⓩA = 0 ∧ B = 0 ∧ C = 1⌝, ⓩA ∉ {B⦂ℤ} ∪ {C}⌝], ⓩTUE = SUN⌝))
] : VC list;
...
\end{document}
=IGN
vc "Δ [B=A]" "$CON x : ℤ ⦁ Δ [x=A ∧ true, true]"; (* wrong *)
=TEX
