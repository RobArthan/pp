...
=SML
set_goal([],ⓩ[X](∀x,y:X ⦁(∃x:X ⦁ x = y))⌝);
a contr_tac; 						
a (z_spec_asm_tac ⓩ ∀ x : X ⦁ ¬ x = y ⌝ ⓩy⌝);	
pop_thm();
=TEX
...
=TEX
=SML
set_goal([],ⓩ{x,y:ℤ | x<y} ⊆ {x,y:ℤ | x<y ∨ x>y+99}⌝);
a(PC_T1"z_library1_ext"prove_tac[]);
=TEX
=GFT ProofPower output
Tactic produced 0 subgoals:
...
=SML
set_goal([],ⓩ(∀x,y,z:ℤ | x ≤ y ∧ x + y < z + x ⦁ x < z)⌝);
a(PC_T1"z_lin_arith"prove_tac[]);
pop_thm();
=TEX
=SML
set_goal([],ⓩ(∀x,z:ℤ | (∃ y:Z ⦁ x ≥ y ∧ ¬ y < z )⦁ x ≥ z)⌝);
a(PC_T1"z_lin_arith"prove_tac[]);
pop_thm();
=TEX
=SML
set_goal([],ⓩ(∀x,y:ℤ | x + 2*y < 2*x ⦁ y + y < x)⌝);
a(PC_T1"z_lin_arith"prove_tac[]);
pop_thm();
=TEX
=SML
...
│true
■
where we are required to demonstrate that $CONSTSPEC$ applied to something in  $T$ is in the set $ℕ$. From $z_fun_∈_clauses$ we have that given an
=INLINEFT
f ∈ X ↠ Y
=TEX
\ and an
...
set_goal([], ⓩ(∀ x:ℙ⋎1 T⦁ CONSTSPEC x ∈ ℕ)⌝);
a(REPEAT strip_tac);
=GFT ProofPower output
...
(*  1 *)  ⓩx ∈ ℙ⋎1 T⌝
...
=SML
set_goal([],ⓩ(∀x,y :ℤ | x  ≤ y ⦁ P (x,y)) ∧ x = y ⇒ P (x,y)⌝);
a(REPEAT strip_tac);
=TEX
=GFT ProofPower output
...
...
assumption of the form $m\ ∈\ ℕ$ so that you can forward chain, with, say, $fc_tac$, to obtain the result that
=INLINEFT
fact (m + 1) = (m + 1) * fact m
=TEX
...
│SOMEPROPERTY : X → ℙ Y
├
│∀ x : X; y : Y  ⦁ y ∈ SOMEPROPERTY x ⇔ SOMEPREDICATE
=TEX
...
=GFT
ALL_FC_T1 fc_⇔_canon rewrite_tac[z_get_specⓩSOMEPROPERTY⌝]
=TEX
\item
...
=GFT
⊕_lemma
[X, Y, Z](∀f : X → Y → Z; x:X;y2:Y;y1:𝕌;z:𝕌| ¬y2=y1⦁
...
$z_⊕_↦_app_thm$ and $z_⊕_↦_app_thm1$:
=GFT
z_⊕_↦_app_thm
...
=SML
set_goal([],ⓩ(STATE ≜ Boolean true, NEXTSWITCH ≜ 2) ∈ SWITCHDATA⌝);
=TEX
...
{\it z\_ℝ\_lin_arith} & This is a complete proof context whose main purpose
...
│     ∀ A, B : REAL_EGoANGLE
│       | A +⋎R REAL_EGoANGLEvDELTA <⋎R B ∧ A +⋎R REAL_EGoANGLEvDELTA real_less B = TRUE
│       ⦁ A <⋎R A +⋎R REAL_EGoANGLEvDELTA ∧ A +⋎R REAL_EGoANGLEvDELTA <⋎R B
=TEX
=GFT
│ vcREAL_EGoINTERPOLATE_2 ?⊢
│     ∀ A, B, C : REAL_EGoANGLE
│       | A +⋎R REAL_EGoANGLEvDELTA <⋎R B ∧ A +⋎R REAL_EGoANGLEvDELTA real_less B = FALSE
│       ⦁ A <⋎R C ∧ C <⋎R B
=TEX
...
=SML
a(rewrite_tac[z_get_specⓩREAL_EGoANGLEvDELTA⌝]);
=TEX
This results in:
=GFT
...

(* ?⊢ *)  ⓩreal 0 <⋎R 1.0 /⋎R 360.0⌝
=TEX
Using the computational rules in the proof context {\it 'z\_ℝ\_lin\_arith} complete the proof:
=SML
...
┌ DO_DIGIT ───────────
│ GVoDISPLAY⋎0, GVoDISPLAY : ℤ;
│ GVoIN_NUMBER⋎0, GVoIN_NUMBER : BOOLEAN;
│ D : TCoDIGIT
├──────
│	GVoIN_NUMBER⋎0 = TRUE ⇒ GVoDISPLAY = GVoDISPLAY⋎0*TCoBASE + D;
│	GVoIN_NUMBER⋎0 = FALSE ⇒  GVoDISPLAY = D;
│	GVoIN_NUMBER = TRUE
└──────────────
...
┌ DO_UNARY_OPERATION ───────────
│ GVoACCUMULATOR⋎0, GVoACCUMULATOR : ℤ;
│ GVoDISPLAY⋎0, GVoDISPLAY : ℤ;
│ GVoLAST_OP⋎0, GVoLAST_OP : ℤ;
│ GVoIN_NUMBER : BOOLEAN;
│ O : UNARY
├──────
│	GVoIN_NUMBER = FALSE;
│	GVoACCUMULATOR = GVoACCUMULATOR⋎0;
│	GVoLAST_OP = GVoLAST_OP⋎0;
│	O = TCoCHANGE_SIGN ⇒ GVoDISPLAY = ~GVoDISPLAY⋎0;
│	O = TCoFACTORIAL ∧ GVoDISPLAY⋎0 ≥ 0 ⇒
│			GVoDISPLAY = fact GVoDISPLAY⋎0;
│	O = TCoSQUARE_ROOT  ∧ GVoDISPLAY⋎0 ≥ 0 ⇒
│			GVoDISPLAY ** 2 ≤ GVoDISPLAY⋎0 < (GVoDISPLAY + 1) ** 2
└──────────────
\pagebreak
...
│	GVoLAST_OP = O;
│	GVoLAST_OP⋎0 = TCoEQUALS ⇒
│			GVoACCUMULATOR = GVoDISPLAY⋎0;
│	GVoLAST_OP⋎0 = TCoPLUS ⇒
│			GVoACCUMULATOR = GVoACCUMULATOR⋎0 + GVoDISPLAY⋎0;
│	GVoLAST_OP⋎0 = TCoMINUS ⇒
│			GVoACCUMULATOR = GVoACCUMULATOR⋎0 - GVoDISPLAY⋎0;
│	GVoLAST_OP⋎0 = TCoTIMES ⇒
│			GVoACCUMULATOR = GVoACCUMULATOR⋎0 * GVoDISPLAY⋎0
└──────────────
...
=GFT
vcOPS_2
	∀ GVoDISPLAY, GVoDISPLAY⋎0 : TCoNUMBER;
...
=GFT
vc2002_2
∀ HI : INTEGER; RESULT, RESULT⋎0 : NATURAL; M : NATURAL
...
=GFT
∀ GVoDISPLAY : TCoNUMBER; GVoIN_NUMBER : BOOLEAN; D : TCoDIGIT
...
(* 10 *)  ⓩ~ 999999 ≤ GVoACCUMULATOR⌝
(*  9 *)  ⓩGVoACCUMULATOR ≤ 999999⌝
(*  8 *)  ⓩ~ 999999 ≤ GVoDISPLAY⌝
(*  7 *)  ⓩGVoDISPLAY ≤ 999999⌝
(*  6 *)  ⓩ0 ≤ GVoLAST_OP⌝
(*  5 *)  ⓩGVoLAST_OP ≤ 6⌝
...
(* ?⊢ *)  ⓩ0 ≤ GVoDISPLAY ⇒ FACT GVoDISPLAY = fact GVoDISPLAY⌝
...
(* ?⊢ *)  ⓩFACT GVoDISPLAY = fact GVoDISPLAY⌝
=TEX
...
open_theory"EX'proc";
z_output_theory{theory="EX'proc",out_file="usr503x1.thl.pp"};
open_theory"REAL_EG'spec";
z_output_theory{theory="REAL_EG'spec",out_file="usr503x2.thl.pp"};
open_theory"REAL_EG'body";
z_output_theory{theory="REAL_EG'body",out_file="usr503x3.thl.pp"};
open_theory"REAL_EGoINTERPOLATE'proc";
z_output_theory{theory="REAL_EGoINTERPOLATE'proc",out_file="usr503x4.thl.pp"};
open_theory"TC'spec";
z_output_theory{theory="TC'spec",out_file="usr503x5.thl.pp"};
open_theory"GV'spec";
z_output_theory{theory="GV'spec",out_file="usr503x6.thl.pp"};
open_theory"calc_prelims";
z_output_theory{theory="calc_prelims",out_file="usr503x7.thl.pp"};
open_theory"OPS'spec";
z_output_theory{theory="OPS'spec",out_file="usr503x8.thl.pp"};
open_theory"OPS'body";
z_output_theory{theory="OPS'body",out_file="usr503x9.thl.pp"};
open_theory"OPSoDIGIT_BUTTON'proc";
z_output_theory{theory="OPSoDIGIT_BUTTON'proc",out_file="usr503x10.thl.pp"};
open_theory"OPSoOPERATION_BUTTON'proc";
z_output_theory{theory="OPSoOPERATION_BUTTON'proc",out_file="usr503x11.thl.pp"};
open_theory"OPSoOPERATION_BUTTONoFACT'func";
z_output_theory{theory="OPSoOPERATION_BUTTONoFACT'func",out_file="usr503x12.thl.pp"};
open_theory"OPSoOPERATION_BUTTONoSQRT'func";
z_output_theory{theory="OPSoOPERATION_BUTTONoSQRT'func",out_file="usr503x13.thl.pp"};
=TEX
}
