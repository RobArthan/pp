...
\TPPtitle{Implementation of Document Handling Commands}
...
\item[Issue 1.1 (1994/05/10)] First version based on \cite{DS/FMU/IED/IMP055}. The difference is that on Solaris 2, saving the tty settings prior to calling LaTeX causes the processing to wait on the terminal when run as a background task.
...
\item[2015/03/14]
Now defines macros \verb"\VCVersion" and \verb"\VCDate".
\item[2015/04/15]
...
EXITSTATUS=0
=TEX
{\tt git log} produces no output and does not return an error response
if a newly added file has never been committed.
=DUMPMORE doctex
vcdefs(){
...
\section{PROGRAM PPTEX} 
Shell script to strip {\tt.pp} files creating \LaTeX{} {\tt.tex} files.

=DUMP pptex
#! /bin/sh
#	pptex   From: $Id: imp100.doc,v 1.25 2009/09/06 13:02:41 rda Exp rda $
#
# Shell script to strip ".pp" files creating ".tex" files.

USAGE="$PROGNAME [-v] [-e edit_script] [-f view_file] [-K] [-k keyword_file] <file> ..."
USAGEMSG="$PROGNAME: usage: $USAGE"
=SH
cat pptex_common >>pptex
=DUMPMORE pptex
VERBOSE="n"
EDITFILE=""
SIEVEFILE=""
...
do
	...
	if test "$VERBOSE" = "y"
	then
		diag $PROGNAME: reading $ppfile writing $outfile
	fi

	if test ! -r "$ppfile"
	then
		diag "$PROGNAME:" $ppfile "does not exist or cannot be read"
		EXITSTATUS=1
	else
		vcdefs $ppfile > $outfile
		eval sieve -u "$OPT_K" "$KEYFILE" "$SIEVEFILE" tex < "$ppfile" >> "$outfile"
		if test "$?" -ne 0
		then	EXITSTATUS=1
		elif test ! -s "$outfile"
		then
...
\newpage
\section{PROGRAM PPTCH}

Shell script to strip {\tt.pp} files creating theory checking
files {\tt.tch} files.

=DUMP pptch
#! /bin/sh
#	pptch   From: $Id: imp100.doc,v 1.25 2009/09/06 13:02:41 rda Exp rda $

PROGNAME=`basename "$0"`
USAGE="$PROGNAME [-v] [-f view_file] [-K] [-k keyword_file] <file> ..."
USAGEMSG="$PROGNAME: usage: $USAGE"
=SH
cat pptex_common >>pptch
=DUMPMORE pptch
VERBOSE="n"
SIEVEFILE=""
KEYFILE=""
...
do
	...
	then
		...
		(	echo 'use_file "dtd035";'
			...
			echo 'req_flag := Check;'
			echo 'initialise_td_results ();'
			eval sieve -u "$OPT_K" "$KEYFILE" "$SIEVEFILE" thydoc < "$ppfile"
			echo 'summarize_td_results ();'
		) > "$outfile"
	fi
done
...
\newpage
\section{PROGRAM PPTDS}

Shell script to strip {\tt.pp} files creating theory design
{\tt.tds} files.

=DUMP pptds
#! /bin/sh
#	pptds   From: $Id: imp100.doc,v 1.25 2009/09/06 13:02:41 rda Exp rda $

PROGNAME=`basename $0`
USAGE="$PROGNAME [-v] [-f view_file] [-K] [-k keyword_file] <file> ..."
USAGEMSG="$PROGNAME: usage: $USAGE"
=SH
cat pptex_common >>pptds
=DUMPMORE pptds
VERBOSE="n"
SIEVEFILE=""
KEYFILE=""
...
\newpage
\section{PROGRAM PPDVI}

Shell script that combines the actions of {\tt pptex}, {\tt bibtex}
(which is part of the basic \TeX{} distribution) and {\tt texdvi} with
...
=DUMP ppdvi
#! /bin/sh
#	ppdvi   From: $Id: imp100.doc,v 1.25 2009/09/06 13:02:41 rda Exp rda $
...
do
	...
	if test $VERBOSE = "y"
	then
		diag $PROGNAME: reading $ppfile writing $dvifile
		SVARGS=$SVARGS" -v"
		TEXARGS=$TEXARGS" -v"
	fi

	eval pptex "$EDITFILE" "$SVARGS" \""$ppfile"\" || { EX=$? ; EXITSTATUS=1 ; continue ; }
	eval texdvi -b "$TEXARGS" "$stem" < /dev/null || { EX=$? ; EXITSTATUS=1 ; continue ; }
	if	test $DOLATEX -ge 2
	then	eval texdvi "$TEXARGS" \""$stem"\" < /dev/null \
		|| { EX=$? ; EXITSTATUS=1 ; continue ; }
...
=DUMP pppr
#! /bin/sh
#	pppr   From: $Id: imp100.doc,v 1.25 2009/09/06 13:02:41 rda Exp rda $

PROGNAME=`basename "$0"`
USAGE="$PROGNAME [-n] [-p] [-s] [-v] [-w width] <filename> ..."
USAGEMSG="$PROGNAME : usage: $USAGE"
=SH
cat pptex_common >>pppr
=DUMPMORE pppr
VERBOSE="n"
SCREENVIEW="n"
PRESERVE="n"
