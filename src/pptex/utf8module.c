...
int wstr_match(wchar_t *prefix, wchar_t *str)
{
	int i = 0;
...
get_real_name(char *name)
{
	...
	if (loops == MAX_LINKS || getcwd(buf, MAX_FILE_NAME_LEN) == NULL || strlen(buf) == MAX_FILE_NAME_LEN + 1)
	{
		if (dir)
		{
			free(dir);
			  ...
			  int show_line)
{
	...
	if (show_line)
	{
		...
		if (file_F->file_name != 0)
			FPRINTF(stderr, " %s", file_F->file_name);
	}
	WFPRINTF(stderr, L": %s\n", msg);
	if (show_line)
		FPRINTF(stderr, "\t%S\n", file_F->cur_line);
			  ...
			  int show_line)
{
	...
	if (show_line)
	{
		...
		if (file_F->file_name != 0)
			WFPRINTF(stderr, L" %s", file_F->file_name);
	}
	PUTWC(L':', stderr);
	PUTWC(L' ', stderr);
...
int get_char_unicode(wchar_t *line, unicode *value)
{
...
void get_tex_arg(wchar_t *macro, regex_t **tex_arg, char *tex_arg_sense)
{
	wchar_t *p = macro;
...
int simple_wread_ext_line(wchar_t *line, int max_len, struct file_data *file_F)
{
	wchar_t whatgot;
	int i = 0;
...
void read_steering_line(wchar_t *line, struct file_data *file_F)
{
	...
	do
	{
		len_read = simple_wread_line(&line[len_so_far],
									 MAX_LINE_LEN - len_so_far, file_F) -
				   1;
		len_so_far += len_read;
...
void read_keyword_file(char *name)
{
	...
	while ((!feof(keyword_F.fp)) && (!ferror(keyword_F.fp)))
	{
		...
		if (wcscmp(kind_str, L"simple") == 0)
			kind = KW_SIMPLE;
		else if (wcscmp(kind_str, L"index") == 0)
			kind = KW_INDEX;
		else if (wcscmp(kind_str, L"directive") == 0)
			kind = KW_DIRECTIVE;
		else if (wcscmp(kind_str, L"startdirective") == 0)
			kind = KW_START_DIR;
		else if (wcscmp(kind_str, L"sameas") == 0)
			kind = KW_SAMEAS;
		else if (wcscmp(kind_str, L"verbalone") == 0)
			kind = KW_VERB_ALONE;
		else if (wcscmp(kind_str, L"white") == 0)
			kind = KW_WHITE;
		else
		{
		...
		else
		{
			...
			if ((kind == KW_DIRECTIVE || kind == KW_START_DIR) && ech == -1)
			{
	...
	*/
	...
	for (i = 1; i < kwi.num_keywords; i++)
	{
		...
		if (prev_ki != NULL && (wcscmp(prev_ki->name, cur_ki->name) == 0))
		{
			...
			{
				if (prev_ki->ech != NOT_FOUND && cur_ki->uni != U_NOT_FOUND && cur_ki->uni != prev_ki->uni)
	...
	for (i = 1; i < kwi.num_keywords; i++)
	{
		...
		case KW_SAMEAS_UNKNOWN:
		{
			...
			{
				struct keyword_information *copy_from = &kwi.keyword[copy_from_index];
			   ...
			   struct file_data *file_F)
{
	...
	kw[kwlen++] = L'%';
	while (kwlen <= MAX_KW_LEN && (ch = str[kwlen]) != L'\0' && ch != L'%' && my_iswascii(ch) && iswgraph(ch))
	{
		kw[kwlen++] = ch;
	}
				   ...
				   struct file_data *file_F)
{
	...
	kw[kwlen++] = L'%';
	while (kwlen <= MAX_KW_LEN && (ch = str[kwlen]) != 0 && ch != L'%' && my_iswascii(ch) && iswgraph(ch))
	{
		kw[kwlen++] = ch;
	}
...
char *my_setlocale(int cat, bool utf8_flag)
{
	char *locres;
	...
	if (utf8_flag)
	{
		...
		{
			locres = setlocale(cat, NULL);
			PRINTF("Failed to set locale to %s (it is %s)", UTF8LOCNAME, locres);
			EXIT(42);
	...
	else
	{
		...
		locres = setlocale(cat, EXTLOCNAME2);
		if (locres)
		{
			return (locres);
		}

		locres = setlocale(cat, EXTLOCNAME3);
		if (locres)
		{
			return (locres);
		}

		/* All EXT locales failed */
		locres = setlocale(cat, NULL);
		PRINTF("Failed to set any EXT locale (%s, %s, %s) - current locale is %s",
			   EXTLOCNAME1, EXTLOCNAME2, EXTLOCNAME3, locres);
		EXIT(42);
		return NULL; /* not reached */
	}
}
...
wchar_t *unicode_to_aekw(unicode code_point)
{
	...
	kwi = unicode_to_kwi(code_point);
	if (kwi == NULL)
		return unicode_to_hex(code_point);
...
wchar_t *unicode_to_kwa(unicode code_point)
{
	...
	if (code_point <= 0x7F)
	{
		...
		return unicode_to_kw(code_point);
}
...
unicode extract_code_point(char *buff, int *pos)
{
	...
	if (l > 4)
	{
		message1("extract_code_point 3 l>4");
		(*pos)--;
...
unicode wextract_code_point(wchar_t *buff, int *pos)
{
	...
	if (l > 4)
	{
		message1("wextract_code_point 3 l>4");
		return invalid_unicode();
	};
	r = r >> l;
	while (--l)
...
int read_line_as_ext(struct file_data *file_F)
{
	...
	if (file_F->utf8)
	{
		...
		if (debug & D_UTF8)
			(void)printf("read_line_as_ext: r=%d eof=%s line=%S\n",
						 r,
						 feof(file_F->fp) ? "Y" : "N",
						 file_F->code_line);
	}
	...
	{
		r = simple_wread_ext_line(file_F->cur_line, MAX_LINE_LEN, file_F);
	};
	return r;
};
...
unicode *utf8_seq_to_unicode(char *utf8seq)
{
	...
	if (debug & D_UTF8)
	{
		PRINTF("utf8_seq_to_unicode: utf8seq = %s, pos = %i\n", utf8seq, pos);
	};
	if (utf8seq == NULL)
		return NULL;
	while (utf8seq[pos] > 0)
	{
		(void)extract_code_point(utf8seq, &pos);
		len++;
	};
	...
	if (debug & D_UTF8)
	{
		WPRINTF(L"utf8_seq_to_unicode: out = %S\n", out);
	};
	return out;
}
...
unicode ext_or_kw_to_unicode(wchar_t *line, int *len)
{
	...
	*len = 0;
	if (line[0] != L'%')
	{
		...
		*len = 1;
		if (line[0] & 0x80)
			return (ext_to_unicode(*line));
		else
			return line[0];
	};
	val = kw_to_unicode(line, len);
	if (*len == 0)
		*len = 1; /* a percent not followed by a keyword */
	return val;
}
