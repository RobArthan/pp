...
In this version the scope is extended to support the use in \Product{} documents of keywords based on the named character entities in HTML5 and MathML.
...
=ENDDOC
=DOC
val ⦏unicode_to_hex_string⦎ : UNICODE -> string
=DESCRIBE
{\it unicode\_to\_hex\_string} encodes a Unicode code point as a hex string. It raises an exception
if its parameter is greater than the maximum Unicode code point (0wx10FFFF).
=FAILURE
122001	Invalid Unicode code point
=ENDDOC
