...
********************************************************************************
%  mdt019.doc  ℤ $Date: 2005/05/07 12:08:26 $ $Revision: 1.21 $ $RCSfile: mdt019.doc,v $
=TEX
...
store_mt_results_show (mt_runf tm_eq)
[
 ("Parser.1.3.1", test_parse, ⌜λx⦁y⌝, Binder("λ", x, y) ),
 ("Parser.1.3.2a", test_parse, ⌜λx:'a⦁y⌝, Binder("λ", xty(TyAtom"'a"), y) ),
 ("Parser.1.3.3", test_parse, ⌜λ(x:'a)⦁y⌝, Binder("λ", xty(TyAtom"'a"), y) ),
 ("Parser.1.3.4", test_parse, ⌜λ(x:'a, y)⦁y⌝, Binder("λ",pair(xty(TyAtom"'a")) y, y) ),
 ("Parser.1.3.5", test_parse, ⌜λ(x, y, z)⦁y⌝, Binder("λ",pair x (pair y z), y) ),
 ("Parser.1.3.2b", test_parse, ⌜λ((x, y), z)⦁y⌝, Binder("λ", pair (pair x y) z, y) )
];
=TEX
...
[
 ...
 ("Parser.1.4.3", test_parse, ⌜let d x = y in z⌝,
	Let([(d, Binder("λ", x, y))], z) ),
 ("Parser.1.4.4", test_parse, ⌜let d x y = y in z⌝,
	Let([(d, Binder("λ", x, Binder("λ", y, y)))], z) ),
 ("Parser.1.4.5", test_parse, ⌜let d (a, b) (y, z) (c, d) = y in z⌝,
	Let([(d, Binder("λ", pair a b, Binder("λ", pair y z, Binder("λ", pair c d, y))))], z) )
];
=TEX
\subsection{Group 2}
...
store_mt_results_show (mt_runf tm_eq)
[
 ("Parser.3.1b", test_parse, ⌜λx y⦁x⌝, test_parse⌜λx⦁λy⦁x⌝),
 ("Parser.3.2b", test_parse, ⌜λx y:'a⦁x⌝, test_parse⌜λx:'a⦁λy:'a⦁x⌝),
 ("Parser.3.3b", test_parse, ⌜λx y:'a; a b:'b⦁x⌝,
	test_parse⌜λx:'a⦁λy:'a⦁λa:'b⦁λb:'b⦁x⌝),
 ("Parser.3.4a", test_parse, ⌜let f (a, b) (x : 'a)= y in f⌝,
	test_parse⌜let f = λ(a, b); x : 'a⦁y in f⌝)
];
=TEX
...
store_mt_results_show mt_run
[
 ("Parser.4.1.1c", test_parse_fail, ⌜λx y⦁⌝, failed_ok),
 ("Parser.4.1.2c", test_parse_fail, ⌜λ⦁y⌝, failed_ok),
