...
********************************************************************************
%  dtd045.doc  ℤ $Date: 2002/10/17 16:20:01 $ $Revision: 1.20 $ $RCSfile: dtd045.doc,v $
=TEX
...
by the predicate HOL constant $⌜⦏IsOneRep⦎⌝$.
=ENDDOC
=THDOC
req_type ("ONE",0);
...
=DESCRIBE
The type $⦏ONE⦎$ has only one member,$⌜⦏One⦎⌝$, demonstrated by its definition ``$⦏One⦎$''
(which is bound to the ML value $one\_def$).
The definition of the type is saved with key ``⦏ONE⦎''.
=ENDDOC
=THDOC
req_thm("one_fns_thm",([],⌜∀ f: 'a → ONE ⦁ f = λ x ⦁ One⌝));
=DESCRIBE
...
=DESCRIBE
The HOL constant $⌜⦏CombS⦎⌝$ is the duplication combinator,
$⌜⦏CombK⦎⌝$ is the deletion combinator,
and $⌜⦏CombI⦎⌝$ is the identity combinator.
