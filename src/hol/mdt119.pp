...
("119.dest_quant.1.2",
	dest_quant,
		⌜∀(x, (y, z))⦁x = y ∧ y = z ∧ z = 1⌝,
			(⌜$∀:(ℕ × ℕ × ℕ → BOOL) → BOOL⌝, ⌜(x:ℕ, (y:ℕ, z:ℕ))⌝, ⌜x = y ∧ y = z ∧ z = 1⌝)),
("119.dest_quant.1.3",
	dest_quant,
		⌜∃⋎1(x, y)⦁x = y ∧ y = 1⌝,
			(⌜$∃⋎1:(ℕ × ℕ → BOOL) → BOOL⌝, ⌜(x:ℕ, y:ℕ)⌝, ⌜x = y ∧ y = 1⌝))
];
=TEX
=SML
store_mt_results
...
("119.dest_quant.2.1",
	dest_quant,
		⌜λx⦁x = x⌝,
			gen_term_fail_msg "dest_quant" 119001 [⌜λx⦁x = x⌝]),
("119.dest_quant.2.2",
	dest_quant,
		⌜CombI(λx⦁x = x)⌝,
			gen_term_fail_msg "dest_quant" 119001 [⌜CombI(λx⦁x = x)⌝])
];
=TEX
=SML
store_mt_results
