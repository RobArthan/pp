...
\usepackage{epsf}
\def\Matrixx{$\mbox{\sf MATRIX}_{\mbox{\sf X}}$}
\def\Xpp{{\tt xpp}}

\def\Title{\parbox[t]{3.25in}{%
\begin{tabular}{@{}c@{}}
{\bf Commercial in Confidence}\\
Scripts Used in Building The Reference Manuals
\end{tabular}}}
\def\FrontPageTitle{{\huge ProofPower Exploitation\\
---\\
Scripts Used in Building The Reference Manuals}}
\def\FrontPageHeader{\raisebox{10ex}{\begin{tabular}[t]{c}{\bf Commercial in Confidence}\end{tabular}}}
\def\Version{\VCVersion}
\def\Abstract{This document contains material to reproduce scripts previously included explicitly in the documentation makefiles.}
\def\Author{G.M. Prout}
\def\Date{\FormatDate{\VCDate}}
\def\Reference{HOL/IMP113}
...
\begin{description}
\item[Issues 1.1 (2000/07/20)-1.6 (2002/06/03)] Initial development drafts.
...
\item[Issue 1.10 (2007/09/09)] New section for order utilities.
\item[Issue 1.11 (2011/07/22)] Allowed for underlining brackets operator.
\item[Issues 1.12 (2013/08/02), 1.13 (2013/08/02)] New section for search trees.
\item[2014/07/23]
...
makefiles {\tt hol.mkf} and {\tt zed.mkf} in the building of reference manual entries in \cite{DS/FMU/IED/USR006}.
...
Scripts provided here were previously generated in the makefiles {\tt hol.mkf} and {\tt zed.mkf} by the {\tt echo} command. This caused problems because {\tt echo} is operating system dependent, in particular in relation to double backslashes.
...
This file contains a number of scripts to be extracted using the docsml program. The resultant file imp113.sml will be empty.
...
\section{FILES}
\subsection{0.15_sed}
File {\tt 0.15_sed} changes the format of the text generated by {\tt pp_list} which surrounds the listing of each theory.
...
File {\tt 0.idx_sed1} takes lines which include index entries and strips out everything except those
...
\subsection{0.ptx_sed4}

File {\tt 0.ptx_sed4} strips out the original index entry from the front of the formatted index entry line.
...
/^signature ⦏UtilitySharedTypes⦎ = sig/,/^end.*signature/w 1a.scr
/^signature ⦏ListUtilities⦎ = sig/,/^end.*signature/w 1b.scr
/^signature ⦏FunctionUtilities⦎ = sig/,/^end.*signature/w 1c.scr
/^signature ⦏Combinators⦎ = sig/,/^end.*signature/w 1d.scr
/^signature ⦏SimpleOutput⦎ = sig/,/^end.*signature/w 1e.scr
/^signature ⦏CharacterUtilities⦎ = sig/,/^end.*signature/w 1f.scr
/^signature ⦏SimpleDictionary⦎ = sig/,/^end.*signature/w 1g.scr
=TEX
...
/^signature ⦏EfficientDictionary⦎ = sig/,/^end.*signature/w 1h.scr
/^signature ⦏Profiling⦎ = sig/,/^end.*signature/w 1i.scr
/^signature ⦏Timing⦎ = sig/,/^end.*signature/w 1j.scr
/^signature ⦏Order⦎ = sig/,/^end.*signature/w 1p.scr
/^signature ⦏Sort⦎ = sig/,/^end.*signature/w 1k.scr
/^signature ⦏SparseArray⦎ = sig/,/^end.*signature/w 1l.scr
/^signature ⦏DynamicArray⦎ = sig/,/^end.*signature/w 1m.scr
/^signature ⦏Integer⦎ = sig/,/^end.*signature/w 1n.scr
/^signature ⦏OrderedEfficientDictionary⦎ = sig/,/^end.*signature/w 1o.scr
/^signature ⦏SearchTree⦎ = sig/,/^end.*signature/w 1q.scr
=TEX
...
/^signature ⦏BasicError⦎ = sig/,/^end.*signature/w 2a.scr
/^signature ⦏SystemControl⦎ = sig/,/^end.*signature/w 2b.scr
=TEX
...
/^signature ⦏KernelTypes⦎ = sig/,/^end.*KernelTypes/w 4a.scr
=TEX
