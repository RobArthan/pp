...
********************************************************************************
%  mdt014.doc  ℤ $Date: 2002/10/17 15:10:58 $ $Revision: 2.7 $ $RCSfile: mdt014.doc,v $
=TEX
...
\def\Title{Module Tests for Unification}

\def\AbstractText{This document comprises test cases and test data for Unification}

\def\Reference{DS/FMU/IED/MDT014}
...
%% LaTeX2e port: \TPPtitle{Module Tests for Unification}  %% Mandatory field
%% LaTeX2e port: \TPPref{DS/FMU/IED/MDT014}  %% Mandatory field
...
For the expression
$(λx ⦁ a = x)F$ test that $a$ and $x$ are both of type
$bool$.
...
For the expression
$(λx ⦁ λy ⦁ λz ⦁ x\ z\ (y\ z))$ test that the types of $x$, $y$ and
