...
********************************************************************************
% usr005C.doc   ℤ $Date: 2002/10/17 15:10:58 $ $Revision: 1.4 $ $RCSfile: usr005C.doc,v $
=TEX
...
Conversions supplied with \Product{} normally prove equations whose left hand side is strictly equal to their term argument, though functions which take conversions as arguments (e.g. conversionals) accept equations whose left hand side is $α$-convertible to the term argument.
...
\subsubsection{simple\_λ\_eq\_rule}
=TEX
...
2.	⊢ (λ x⦁ x) = (λ x⦁ x)					refl_conv
3.	⊢ ((λ x⦁ x) = (λ x⦁ x)) ⇒ ((λ x⦁ x) = (λ x⦁ x))		⇒_intro 2
4.	⊢ ((λ x⦁ x) = (λ x⦁ x)) ⇔ T				eq_sym_rule 1.
5.	⊢ ((λ x⦁ x) = (λ x⦁ x)) ⇒ T				subst_rule 1,4
6.	⊢ T								⇒_elim
=TEX
...
=GFT Informal Proof
1.	Γ ⊢ t ⇔ T						Hypothesis
2.	Γ ⊢ T ⇔ t						eq_sym_rule 1
3.	⊢ T							t_thm
4.	Γ ⊢ t							subst_rule 3,2
=TEX
...
=GFT
1.	⊢ $∀ = (λP ⦁ P = λx.T)				inst_type applied to Definition
2.	Γ ⊢ ∀ x ⦁ t2[x]					Hypothesis
3.	Γ ⊢ (λP ⦁ P = λx.T)(λ x ⦁ t2[x])			subst_rule 1,2
4.	⊢ (λP ⦁ P = λx ⦁ T)(λ x ⦁ t2[x]) = ((λ x ⦁ t2[x]) = λx.T)
								simple_β_conv
5.	Γ ⊢ (λ x ⦁ t2[x]) = λx ⦁ T				subst_rule 4,3
6.	⊢ (λ x ⦁ t2[x]) t1 ⇔ (λ x ⦁ t2[x]) t1		eq_refl_conv
7.	Γ ⊢ (λ x ⦁ t2[x]) t1 ⇔ (λx ⦁ T) t1			subst_rule 5,6
8.	Γ ⊢ (λ x ⦁ t2[x]) t1 ⇔ t2'[t1]			simple_β_conv
9.	Γ ⊢ t2'[t1] ⇔ (λ x ⦁ t2[x]) t1			eq_sym_rule 8
10.	Γ ⊢ t2'[t1] ⇔ (λx ⦁ T) t1				subst_rule 9,5
11.	⊢ (λx ⦁ T) t1 ⇔ T					simple_β_conv
12.	Γ ⊢ t2'[t1] ⇔ T					subst_rule 10,11
13.	Γ ⊢ t2'[t1]						⇔_T_elim 12
=TEX
...
4.	Γ1 ∪ Γ2 ⊢ u1 v1 = u2 v2			subst_rule [1,2] 3
=TEX
...
1.	Γ1 ⊢ t1 ⇔ t2						Hypothesis
2.	Γ2 ⊢ t1'						Hypothesis
3.	Γ2 ⊢ t1' ⇔ T					⇔_T_intro 2
4.	Γ1 ∪ Γ2 ⊢ t2 ⇔ T					subst_rule 1,3
5.	Γ1 ∪ Γ2 ⊢ t2					⇔_T_elim 4
=TEX
...
First we prove $∀x ⦁ T$
=GFT Informal Proof
1.	⊢ (λx ⦁ T) = λx ⦁ T				eq_refl_conv
2.	⊢ ((λx ⦁ T) = λx ⦁ T) ⇔ T			⇔_T_intro 1
3.	⊢ (λP ⦁ P = λx ⦁ T)(λx ⦁ T)
		⇔ ((λx ⦁ T) = λx ⦁ T)			simple_β_conv
4.	⊢ (λP ⦁ P = λx ⦁ T)(λx ⦁ T) ⇔ T		subst_rule 3, 2
5.	⊢ $∀ = (λP ⦁ P = λx ⦁ T)				inst_type applied to Definition
6.	⊢ (λP ⦁ P = λx ⦁ T) = $∀				eq_sym_rule 5
...
1.	Γ ⊢ t							Hypothesis
2.	Γ ⊢ t ⇔ T						⇔_T_intro 1
3.	Γ ⊢ (λy ⦁ t) = λy ⦁ T				abs_rule 2
4.	⊢ (∀y ⦁ t) ⇔ (∀y ⦁ t)				eq_refl_conv
5.	Γ ⊢ (∀y ⦁ t) ⇔ ∀y ⦁ T				subst_rule 3,4
6.	Γ ⊢ (∀y ⦁ T) ⇔ ∀y ⦁ t				eq_sym_rule 5
...
2.	Γ ⊢ ∀ x1 ... xn ⦁ t[x1, ..., xn]		repeated simple_intro on 1
3.	Γ ⊢ t[t1, ..., tn]				repeated simple_∀_elim on 2
=TEX
...
\footnotetext{Here, weak equality means equality up to $α$-convertibility and order of hypotheses}
