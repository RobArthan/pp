...
********************************************************************************
%  $Id: imp045.doc,v 1.20 2005/12/16 10:34:27 rda Exp rda $ ℤ
=TEX
...
push_goal([],⌜∃ $o ⦁ ∀ (f:'b → 'c) (g:'a → 'b) (x: 'a) ⦁
	(f o g) x = f(g x)⌝);
a(simple_∃_tac ⌜λ (f:'b → 'c) (g:'a → 'b) (x: 'a) ⦁ f(g x)⌝ THEN rewrite_tac[]);
pop_thm()));
=TEX
=SML
...
push_goal([],⌜∃ CombS ⦁ ∀ (f:'a → 'b → 'c) (g:'a → 'b) (x:'a) ⦁
	CombS f g x = f x (g x)⌝);
a(simple_∃_tac ⌜λ (f:'a → 'b → 'c) (g:'a → 'b) (x:'a) ⦁
...
a(simple_∃_tac ⌜λ (x : 'a) (y : 'b) ⦁ x⌝ THEN rewrite_tac[]);
pop_thm()));
=TEX
=SML
val ⦏comb_i_def⦎ = new_spec1(["CombI","comb_i_def"], ["CombI"], (
push_goal([],⌜∃ CombI ⦁ ∀ (x : 'a) ⦁ CombI x = x⌝);
a(simple_∃_tac ⌜λ (x : 'a) ⦁ x⌝ THEN rewrite_tac[]);
pop_thm()));
=TEX
=SML
...
let	val (x,bdy) = dest_simple_∀ tm
	...
	val s1 = simple_∀_elim lxbdy
		(inst_type_rule [(aty,vaty),(bty,vbty)]
		 ∀_sum_thm);
	val s2 = conv_rule (ONCE_MAP_C simple_β_conv) s1;
	val s3 = conv_rule (RAND_C
		(APP_C (RAND_C(RAND_C (simple_α_conv xnm')),
		RAND_C(simple_α_conv xnm''))))
			s2;
