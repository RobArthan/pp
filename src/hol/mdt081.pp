...
store_mt_results_show (mt_runf (fn (i, j) => (i = 0 andalso j = 0) orelse i*j > 0)) [
	...
	("Normalisation.1.1.3", uncurry type_order, (⌜:ℕ + ℕ⌝, ⌜:ℕ + ℕ⌝), 0),
	("Normalisation.1.1.4", uncurry type_order, (⌜:ℕ + ℕ⌝, ⌜:'a⌝), 1)
];
=TEX
=SML
...
fun ⦏test_order_comb⦎ (r : TERM ORDER) : TERM ORDER = (fn tm1 => fn tm2 =>
	integer_order (dest_ℕ tm1) (dest_ℕ tm2)
...
store_mt_results_show (mt_runf (op ~=$))[
	...
	my_concl o poly_conv term_order
		...
		(COND_C (fn t => dest_ℕ t @> one) plus1_conv fail_conv),
...
store_mt_results_show mt_run_fail[
	("Normalisation.8.3.1",
		my_concl o cnf_conv,
		⌜x:ℕ⌝, gen_fail_msg"cnf_conv" 81030 [string_of_term ⌜x⌝]),
	("Normalisation.8.3.2",
		my_concl o cnf_conv,
		⌜x∧y⌝, gen_fail_msg"cnf_conv" 81030 [string_of_term ⌜x∧y⌝]),
	("Normalisation.8.3.3",
		my_concl o dnf_conv,
		⌜x:ℕ⌝, gen_fail_msg"dnf_conv" 81031 [string_of_term ⌜x⌝]),
	("Normalisation.8.3.4",
		my_concl o dnf_conv,
...
store_mt_results_show mt_run_fail[
	("Normalisation.8.4.1",
		my_concl o anf_conv,
		⌜x:ℕ⌝, gen_fail_msg"anf_conv" 81032 [string_of_term ⌜x⌝]),
	("Normalisation.8.4.2",
		my_concl o anf_conv,
