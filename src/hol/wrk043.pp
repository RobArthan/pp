...
********************************************************************************
=TEX
% 199.doc   ℤ $Revision: 2.10 $ $RCSfile: wrk043.doc,v $ $Date: 2010/01/31 15:31:20 $
...
of replacement). The language is at heart just the simply-typed λ-calculus
...
relation in $V$.
Note that the set type constructor ℙ is written as a post-fix operator and
...
(* ?⊢ *)  ⌜∀ (V, E) U
           ⦁ (E ∈ ℙ {(v, w)|v ∈ V ∧ w ∈ V} ∧ (∀ x y⦁ (x, y) ∈ E ⇔ (y, x) ∈ E))
                 ∧ U ⊆ V
               ⇒ {(v, w)|v ∈ U ∧ w ∈ U} ∩ E ∈ ℙ {(v, w)|v ∈ U ∧ w ∈ U}
                 ∧ (∀ x y
                 ⦁ (x, y) ∈ {(v, w)|v ∈ U ∧ w ∈ U} ∩ E
                     ⇔ (y, x) ∈ {(v, w)|v ∈ U ∧ w ∈ U} ∩ E)⌝
=TEX
...
a(strip_asm_tac (∀_elim⌜a⌝ ℕ_cases_thm) THEN
	strip_asm_tac (∀_elim⌜b⌝ ℕ_cases_thm));
(* 4 cases *)
(* *** Goal "2.1.1" *** *)
a(∃_tac⌜0⌝ THEN REPEAT strip_tac);
a(∃_tac⌜{}⌝ THEN REPEAT strip_tac THEN
