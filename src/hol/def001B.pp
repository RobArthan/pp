...
=IGN
 ℤ $Revision: 1.7 $ $RCSfile: def001B.doc,v $ $Date: 2002/10/17 16:20:01 $
=TEX
...
\item The alternatives for $Tm$ above are listed in order of increasing
	...
	in the list above is to be preferred.  For example, the sentence
		`$ λx⦁ t  = u $' would be parsed as
		`$ λx⦁(t  = u)$' rather than as
		`$(λx⦁ t) = u $' by this rule.
...
\item \label{TypeRule} A term containing a type must be parsed so that
	...
	example, `$t:σ⋎1\;σ⋎2:σ⋎3$' must be parsed so that the types it
	contains are `$σ⋎1\;σ⋎2$' and `$σ⋎3$' (not `$σ⋎1$' and `$σ⋎3$').
	This term might be rewritten as `$(t:σ⋎1\;σ⋎2):σ⋎3$' indicating
	that types `$σ⋎1\;σ⋎2$' and `$σ⋎3$' are equivalent, and that
	`$t$' has that type.
...
A paired $λ$-term is a syntactic abbreviation for (i.e. is parsed into the same term as) an application of the constant $Uncurry$ to a simple $λ$-abstraction formed by abstracting over each of the constituent variables in turn.
...
Where a binding construct is formed using a constant declared as a binder, the same effect takes place on the underlying $λ$-term.
...
Where $λ$-terms or nested terms formed by application of a constant (or variable) with lexical status of `binder' appear, the inner occurrences of `$λ$' or of the repeated binder may be omitted.
...
=GFT HOL terms
	...
	⌜∀ (x,y) : (BOOL × BOOL) ⦁ T⌝;
	⌜∀ (x,y) ((z,x⋎1),y⋎1) ⦁ T⌝;
	⌜∀ (x,y) (z,x⋎1) (y⋎1,z⋎1) : BOOL × BOOL; x⋎2 y⋎2; z⋎2 : ℕ ⦁ T⌝;
	⌜∀ x : BOOL SET ⦁ x ⊆ {T,F}⌝;
...
The simple $λ$-abstraction
=INLINEFT
⌜λx⦁t⌝
=TEX
\ yields a term formed by the primitive `$mk\_abs$' constructor with the terms $x$ and $t$ as its arguments.
All variable binding constructs in the \ProductHOL\ concrete syntax are syntactic abbreviations for less concise expressions in which all variable binding is done by simple $λ$-expressions.
...
Where the value to be assigned to the local variable is a function then the value may either be expressed as a $λ$-expression.
...
=GFT HOL terms
	⌜let (a,b) = c in f(a)⌝;
	⌜let (a:ℕ,(b,(c,d),e)) = exp in f(a,b,c,d,e)⌝;
	⌜let x = exp1 in exp2⌝;
...
For a local function definition the function name is followed by formal argument patterns, a sequence of variable structures in the same form as would be permitted in a `$λ$'-expression.
...
=GFT Identical HOL TERMs
	...
	⌜Let(Let(Let(λ x y z ⦁ w)t)u)v⌝;
=TEX

=GFT Identical HOL TERMs
	⌜let x = t in let y = u in let z = v in w⌝;
	⌜Let(λ x ⦁ Let(λ y ⦁ Let(λ z ⦁ w)v)u)t⌝;
=TEX
...
term.  The set  `$\{x|p\}$' stands for `$SetComp(λ x
⦁ p)$.'
