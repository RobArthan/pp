...
********************************************************************************
%  ℤ $Date: 2002/10/17 15:10:58 $ $Revision: 1.13 $ $RCSfile: mdt072.doc,v $
=TEX
...
%% LaTeX2e port: =TEX
%% LaTeX2e port: %  ℤ $Date: 2002/10/17 15:10:58 $ $Revision: 1.13 $ $RCSfile: mdt072.doc,v $
%% LaTeX2e port: =TEX
...
(*	Z:	ⓩ (∃⋎1 y : Y ⦁ (x, y) ∈ f) ⇒ (x, f(x)) ∈ f ∧ (f(x) = μ y : Y | (x, y) ∈ f) ⌝ *)
("60_01a",	⌜ ∀ f : ('a ↔ 'b) ⦁ (∃⋎1 y ⦁ (x, y) ∈ f)
			⇒ (x, f @ x) ∈ f ∧ (f @ x = ε y ⦁ (x, y) ∈ f) ⌝, true ),
...
repeat drop_main_goal;
push_goal(nil, ⌜
	∀ f : ('a ↔ 'b) ⦁ (∃⋎1 y ⦁ (x, y) ∈ f) ⇒ (x, f @ x) ∈ f ∧ (f @ x = ε y ⦁ (x, y) ∈ f)
⌝);
...
(* *** Goal "4.2" *** *)
a(bc_tac [get_spec⌜$At⌝] THEN strip_tac THEN asm_prove_tac nil);
a(spec_nth_asm_tac 4 ⌜(ε y⦁ (x, y) ∈ f)⌝);
a(asm_rewrite_tac nil);
a(asm_prove_tac nil);
