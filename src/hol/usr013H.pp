...
Using $∀_elim$ and $all_β_rule$ these can be specialised for use in forward proofs, however, in practice induction principles are normally used via induction tactics.
...
It takes as a parameter a term which should be a variable of type ⓣℕ⌝ which occurs free in the conclusion of the current goal (but not free in the assumptions).
...
=GFT ProofPower Output
val it = ⊢ (∃ x⦁ φ x) ∨ (∃ y⦁ ψ y) ⇔ (∃ z⦁ φ z ∨ ψ z) : THM
