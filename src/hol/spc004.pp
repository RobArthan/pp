...
type_spec {rep_fun="rep_theory_hierarchy", def_tm = ⌜
	⦏THEORY_HIERARCHY⦎ ≃ ⦏mk_theory_hierarchy⦎ Of wf_hierarchy
⌝};
=TEX
...
=GFT
	λsys:('INPUT, 'OUTPUT, 'STATE)HOL_SYSTEM⦁∀(v:'U)⦁validity_preserving v sys
=TEX
...
The conjecture
=GFT
	?⊢ xx = λsys:('INPUT, 'OUTPUT, 'STATE)HOL_SYSTEM⦁∀(v:'U)⦁
						validity_preserving v sys
=TEX
is equal to $λsys⦁T$ at some instances of $'U$ but not at
