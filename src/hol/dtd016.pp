...
\def\Title{Detailed Design for Type Inference}

\def\AbstractText{This document gives the detailed design of the type inference mechanism for ICL HOL.}

\def\Reference{DS/FMU/IED/DTD016}
...
%% LaTeX2e port: \TPPtitle{Detailed Design for Type Inference}  %% Mandatory field
%% LaTeX2e port: \TPPref{DS/FMU/IED/DTD016}  %% Mandatory field
%% LaTeX2e port: \def\SCCSversion{$Revision: 2.18 $
%% LaTeX2e port: }
%% LaTeX2e port: \TPPissue{\SCCSversion}  %% Mandatory field
%% LaTeX2e port: \TPPdate{\FormatDate{$Date: 2004/06/16 16:33:03 $
...
%% LaTeX2e port: of the type inference mechanism for ICL HOL.
%% LaTeX2e port: }
...
=FRULE 1 Rule
binding constructor is "λ"
├
ENV ⊢ t⋎1 → T⋎1:σ⋎1;

ENV ⊕
{n ↦ VarInfo σ' |
	mk_var(n, σ') ∈ frees T⋎1}
	⊢ t⋎2 → T⋎2:σ⋎2
├
ENV ⊢ Binder ("λ", t⋎1, t⋎2)
	→ mk_λ(T⋎1:σ⋎1, T⋎2:σ⋎2)
=TEX
...
ENV ⊢ Binder (TmAtom (Id s), t⋎1, t⋎2)
  → (mk_app(B:(σ⋎1 → σ⋎2) → σ⋎3,
	mk_λ(T⋎1:σ⋎1,T⋎2:σ⋎))):σ⋎3
=TEX
...
The if-part, ⌜3:ℕ⌝, must have type ⓣBOOL⌝
=TEX
=EXAMPLE
...
⌜1 1⌝;
Type error in ⌜1 1⌝
The operator must have a type of the form σ → τ
Cannot apply ⌜1:ℕ⌝
          to ⌜1:ℕ⌝
=TEX
=EXAMPLE
...
in section \ref{Let}.
=FAILURE
16541	The operator must have a type of the form σ → τ
16542	In ⌜f a⌝ where ⌜a:σ⌝, ⌜f⌝ must have type of the form σ → τ
...
=TEX
=FAILURE
16530	In ⌜t:σ⌝, σ must be an instance of the type of ⌜t⌝
...
the type of free variable ⌜x:ℕ⌝
cannot be instantiated to ⓣBOOL⌝
=EXAMPLE
let val t1 = ⌜x:ℕ→ℕ⌝; val t2 = ⌜x:ℕ⌝
in	⌜ⓜmk_app(t1, t2)⌝⌝
end;
Type error in ⌜<term quotation>⌝
...
Each element in a set display must have the same type $σ$. The overall set
has the type $σSET$
.
=FRULE 1 Rule
├
...
Type ⓣℕ⌝ inferred for the following elements:
   ⌜1⌝
   ⌜2⌝
   ⌜4⌝
...
Each element in a list display must have the same type $σ$. The overall list
has the type $σLIST$
