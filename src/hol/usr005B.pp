...
********************************************************************************
% usr005B.doc   ℤ $Date: 2002/10/17 15:10:58 $ $Revision: 1.3 $ $RCSfile: usr005B.doc,v $
=TEX
\section{Types}
...
\label{Simple Terms}
There are four primitive term constructors in HOL for making variables, constants, function applications and λ-abstractions respectively.
...
\begin{tabular}{|l|l|l|p{2in}|} \hline
	...
	$Simpleλ$		& $⌜λ x⦁ t⌝$		& $mk\_simple\_λ$	& The abstraction of variable $x$ over term $t$. \\ \hline
\end{tabular}
...
causes
=GFT
⊢ ∃ f : ('s, 'f) PAIR → ('f × 's) ⦁ TypeDefn (λ x ⦁ T) f
=TEX
