...
********************************************************************************
%  ℤ $Date: 2008/07/25 14:20:09 $ $Revision: 1.38 $ $RCSfile: imp108.doc,v $
=TEX
...
\def\Title{Portability Infrastructure Implementation}

\def\AbstractText{This document implements the detailed design given in DTD108.}

\def\Reference{DS/FMU/IED/IMP108}
...
%% LaTeX2e port: \TPPtitle{Portability Infrastructure Implementation}  %% Mandatory field
%% LaTeX2e port: \TPPref{DS/FMU/IED/IMP108}  %% Mandatory field
%% LaTeX2e port: \def\SCCSversion{$Revision: 1.38 $%
%% LaTeX2e port: }
%% LaTeX2e port: \TPPissue{\SCCSversion}  %% Mandatory field
%% LaTeX2e port: \TPPdate{\FormatDate{$Date: 2008/07/25 14:20:09 $%
%% LaTeX2e port: }}
...
%% LaTeX2e port: \TPPabstract{This document implements the detailed design given in DTD108.}
...
\item[2016/01/09]
Now uses the Poly/ML {\tt addPrettyPrinter} function rather than the obsolescent {\tt install\_pp}.
...
=POLYML
	PolyML.print_depth;
=TEX
We include a file generated by processing dtd108.doc that (under Poly/ML) defines
a type {\em PP\_INT} which is the one used by the compiler for integers in the {\em pretty}
data type and funtion {\em int\_to\_pp\_int} that converts the default integers to this type.
=INCLUDE
imp108-pp-include.sml
=NJML
type 'a ⦏PP_SPEC⦎ = string list * (PrettyPrint.stream -> 'a -> unit);
=TEX
For Poly/ML as we are only supporting pretty-printing of monotypes here, we instantiate the type variable
that is used to handle the arguments of polymorphic types to {\tt unit}.
=POLYML
type 'a PP_SPEC = (PP_INT -> unit -> 'a -> PolyML.pretty);
=TEX
=NJML
...
=POLYML
(	fn _ => fn () => fn v =>
	let	fun moan s = TextIO.output(TextIO.stdOut,
			"Error in pretty-printer: " ^  s ^ "\n");
		open PolyML;
		val stk : pretty list ref = ref [];
		fun add_entry p = (
			case !stk of 
				[] =>
				stk := [p]
			|	(PrettyBlock(i, b, c, ps) :: more) =>
				stk := PrettyBlock(i, b, c, ps @ [p]) :: more
			|	other => (
				moan "missing begin"
			)
		);
		val add_string : string -> unit = add_entry o PrettyString;
		fun begin_block (i : int, b : bool) : unit = (
			stk := PrettyBlock (int_to_pp_int i, b, [], []) :: !stk
		);
		fun add_break (i : int, j : int) : unit = (
			(add_entry o PrettyBreak)(int_to_pp_int i, int_to_pp_int j)
		);
		fun end_block (() : unit) : unit = (
			case !stk of
				(p as PrettyBlock _) :: more => (
				stk := more;
				add_entry p
			) |	other => (
				moan "end with no matching begin"
			)
		);		
	in	pprint (add_string, begin_block, add_break, end_block) v;
		case !stk of
			[] => PrettyBlock(int_to_pp_int 0, false, [], [])
		|	[p] => p
		|	ps => (
			moan "begin with no matching end";
			PrettyBlock(int_to_pp_int 0, false, [], ps)
		)
	end
);
=SML
...
=MLTON
	val ⦏implode⦎ : string list -> string = foldr (op ^) "";
	val ⦏explode⦎ : string -> string list = map str o String.explode;
	exception ⦏Ord⦎;
	val  ord : string ->  int = fn s => (Char.ord (String.sub(s, 0)) handle Subscript => raise Ord);
	val chr : int -> string = str o Char.chr;
=POLYML
	open SML90;
=NJML
	open SML90;
...
=MLTON
(* MLton does not have a persistent object store etc.
=SML
...
The following hack for MLton is an under-approximation but we don't use
the exceptions that we have left out and nothing depends on the variant
behaviour of the functions on reals. So all we need are the old string
operations and Basic I/O.
=MLTON
structure SML90 = struct
	open PPString;
	open BasicIO;
end (* of structure SML90 *);
=SML
