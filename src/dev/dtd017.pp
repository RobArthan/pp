...
{\bf Abstract}\par
{The detailed design of, SLRP, a parser generator based on the SLR(1)
technique and using the method of Bermudez and Logothetis to implement
LALR(1) via SLR(1).
...
\item[2015/11/21]
Added {\it slrp\_main} to the signature.
...
is a subset of British Standard BNF, \cite{BS6154},
...
unnecessary η-redex. The function
=INLINEFT
slrp'gen_parser
=TEX
...
=ENDDOC
=DOC
val ⦏slrp_main⦎ : unit -> unit;
=DESCRIBE
{\it Slrp.slrp\_main} is used as the entry point when {\it slrp} is compiled as stand-alone binary.
It invokes  {\it Slrp.slrp} with the parameter attributes derived from invariant variables as follows: 
=GFT
SLRPINFILE		in_file			string
SLRPOUTFILE		out_file		string
SLRPEOS			eos			string
SLRPQUOTECON		quote_con		non-empty string or "" for Nil
SLRPNAMECON		name_con		non-empty string or "" for Nil
SLRPLOGFILE		log_file		string
SLRPLOGLEVEL		log_level		decimal integer
SLRPSLRONLY		slr_only		true/false
SLRPTIMINGS		timings			true/false
SLRPGENERIC		generic			true/false
SLRPREDUCTIONSFILE	reductions_file		non-empty string or "" for Nil
SLRPBRIEFINTERFACE	brief_interface		true/false
=ENDDOC
The caller is expected to have validated the parameters:
values other than true or false for boolean parameters are treated as false;
an invalid decimal integer is treated as zero;
an empty string passed as as an optional string will be treateed as {\it Nil}.
=SML
