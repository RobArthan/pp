...
\item[Issue 1.12 (2005/09/09)] BOOL and CHAR are now renamed as 𝔹 and 𝕊.
...
$𝕌$ simplification:
=SML
store_mt_results (mt_runf (op =$)) [("↔ 𝕌 simp",
...
store_mt_results (mt_runf (op =$)) [
	("mk_z_if.1",
		mk_z_if, (ⓩ1 = 2⌝, ⓩ3⌝, ⓩ4⌝),
		ⓩ(if 1 = 2 then 3 else 4) ⦂ 𝕌⌝),
	("mk_z_if.2",
		mk_z_if, (ⓩ1 = 2⌝, ⓩ(3, 4)⌝, ⓩ(5, 6)⌝),
		ⓩ(if 1 = 2 then (3, 4) else (5, 6)) ⦂ 𝕌⌝)
];
=TEX
=SML
store_mt_results (mt_runf (fn ((t1, t2, t3), (u1, u2, u3)) => t1 =$ u1 andalso t2 =$ u2 andalso t3 =$ u3 )) [
	("dest_z_if.1",
		dest_z_if, ⓩ(if 1 = 2 then 3 else 4) ⦂ 𝕌⌝,
			(ⓩ1 = 2⌝, ⓩ3⌝, ⓩ4⌝)),
	("dest_z_if.2",
		dest_z_if, ⓩ(if 1 = 2 then (3, 4) else (5, 6)) ⦂ 𝕌⌝,
			(ⓩ1 = 2⌝, ⓩ(3, 4)⌝, ⓩ(5, 6)⌝))
];
=TEX
=SML
store_mt_results mt_run [
	("is_z_if.1",
		is_z_if, ⓩ(if 1 = 2 then 3 else 4) ⦂ 𝕌⌝, true),
	("is_z_if.2",
