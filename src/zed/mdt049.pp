...
********************************************************************************
%  ℤ $Revision: 1.21 $ $RCSfile: mdt049.doc,v $ $Date: 2011/07/11 11:44:45 $
=TEX
...
=SML
val g1_spec = ⓩG1 = 𝕌⌝;
val tree_type_spec = ⓩTREE = 𝕌⌝;
val tree_cons_spec = ⓩ(tip ∈ TREE
    ∧ fork ∈ ℕ × TREE × TREE ↣ TREE)
    ∧ disjoint ⟨{tip}, ran fork⟩
    ∧ (∀ W : ℙ TREE | {tip} ∪ fork ⦇ ℕ × W × W ⦈ ⊆ W ⦁ TREE ⊆ W)⌝;
val yin_spec = ⓩYIN = 𝕌⌝;
val yang_spec = ⓩYANG = 𝕌⌝;
val yin_yang_cons_spec = ⓩ(YinSeed ∈ YIN
    ...
    ∧ (∀ W1 : ℙ YIN; W2 : ℙ YANG
      ...
      ⦁ YIN ⊆ W1 ∧ YANG ⊆ W2)⌝;
val twice_spec = ⓩ(twice _) ∈ ℤ → ℤ ∧ (∀ i : ℤ ⦁ twice i = 2 * i)⌝;
val swap_spec = ⌜∀ X : 'X SET; Y : 'Y SET⦁ ⓩX swap Y = Y × X⌝⌝;
val sch1_spec = ⓩSch1 = []⌝;
val sch2_spec = ⓩSch2 = [ | false]⌝;
val sch3_spec = ⓩSch3 = [x, y, z : ℤ | x = y ∨ y = z]⌝;
val cycle_spec = ⌜
...
store_mt_results_show (mt_runf (op ~=$)) [
	...
	("mdt049.5.3", Combinators.I, ⓩ[X, Y] X = X⌝,
			⌜∀ X : 'X SET; Y : 'Y SET⦁ X = X⌝),
	("mdt049.5.4", Combinators.I, ⓩ(f 1)⦂ ℤ⌝, mk_z_app(ⓩf⦂ (ℤ → ℤ)⌝, ⓩ1⌝)),
	("mdt049.5.5", Combinators.I, ⓩⓩ(f 1)⌝⦂ ℤ⌝, mk_z_app(ⓩf⦂ (ℤ → ℤ)⌝, ⓩ1⌝)),
	("mdt049.5.6", Combinators.I, ⓩΠ(X)⌝, ⌜X:BOOL⌝)
];
=TEX
\subsection{Group 6}
...
■
=SML
val ijk_spec = ⓩ(IJK ∈ ℤ ∧ ABC ∈ ℤ ∧ DEF ∈ ℤ ∧ LMN ∈ ℤ) ∧ IJK = DEF ∧ ABC ≠ LMN⌝;
store_mt_results_show (mt_runf (op ~=$)) [
