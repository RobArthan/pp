...
********************************************************************************
% imp048.doc ℤ $Date: 2008/03/02 16:13:37 $ $Revision: 1.45 $ $RCSfile: imp048.doc,v $
=TEX
...
\def\Title{Implementation of Support for Z Types and Terms}

\def\AbstractText{}

\def\Reference{DS/FMU/IED/IMP048}
...
%% LaTeX2e port: \TPPtitle{Implementation of Support for Z Types and Terms}  %% Mandatory field
%% LaTeX2e port: \def\TPPheadtitle{Implementation of Support for Z Types and Terms}
%% LaTeX2e port: \TPPref{DS/FMU/IED/IMP048}  %% Mandatory field
%% LaTeX2e port: \def\SCCSversion{$Revision: 1.45 $%
%% LaTeX2e port: }
%% LaTeX2e port: \TPPissue{\SCCSversion}  %% Mandatory field
%% LaTeX2e port: \TPPdate{\FormatDate{$Date: 2008/03/02 16:13:37 $%
%% LaTeX2e port: }}
...
=SML
fun	⦏bind_Δ_name⦎ (fid:string list) :string = "Z'Δ⋎s" ^ (val_set_ident fid);
fun	⦏bind_Δ_type⦎ (itym:(string * TYPE) list) :TYPE =
		mk_→_type(
			schema_type itym,
			schema_type(itym @ (map (fn(id,ty)=>(pack_ident(id, "'"), ty)) itym))
		);
fun	⦏bind_Δ_term⦎ (itym:(string * TYPE) list) :TERM =
		mk_const (bind_Δ_name (map fst itym), bind_Δ_type itym);
=TEX
...
=SML
fun	⦏bind_Ξ_name⦎ (fid:string list) :string = "Z'Ξ⋎s" ^ (val_set_ident fid);
val	⦏bind_Ξ_type⦎ :(string * TYPE) list -> TYPE = bind_Δ_type;
fun	⦏bind_Ξ_term⦎ (itym:(string * TYPE) list) :TERM =
		mk_const (bind_Ξ_name (map fst itym), bind_Ξ_type itym);
=TEX
...
=SML
fun	⦏∃⋎s_bname⦎ (fid1:string list) (fid2:string list) :string =
		"Z'∃⋎s" ^
		(val_set_ident fid1) ^
		(val_set_ident fid2);
fun	⦏∃⋎s_btype⦎ (itym1:(string * TYPE) list) (itym2:(string * TYPE) list) :TYPE =
		list_mk_→_type[
			list_mk_→_type (itymfunseq itym1 @[ g1_bind_type]),
			schema_type itym2,
			schema_type (itym2 diff' itym1)];
fun	⦏∃⋎s_bterm⦎ (itym1:(string * TYPE) list) (itym2:(string * TYPE) list) :TERM =
		mk_const (
...
=SML
fun	⦏∃⋎1⋎s_bname⦎ (fid1:string list) (fid2:string list) :string =
		"Z'∃⋎1⋎s" ^
		(val_set_ident fid1) ^
		(val_set_ident fid2);
fun	⦏∃⋎1⋎s_btype⦎ (itym1:(string * TYPE) list) (itym2:(string * TYPE) list) :TYPE =
		∃⋎s_btype itym1 itym2;
fun	⦏∃⋎1⋎s_bterm⦎ (itym1:(string * TYPE) list) (itym2:(string * TYPE) list) :TERM =
		mk_const (
...
in
fun	⦏∃⋎1_bname⦎ (fid:string list) :string =
		"Z'∃⋎1[" ^ (number(length (idsetseq fid))) ^ "]";
fun	⦏∃⋎1_btype⦎ (itym:(string * TYPE) list) :TYPE =
		mk_→_type
			(list_mk_→_type (itymfunseq itym @[ g2_bind_type_bool]), BOOL);
fun	⦏∃⋎1_bterm⦎ (itym:(string * TYPE) list) :TERM =
...
=SML
fun	⦏λ_bname⦎ (fid:string list) :string =
		"Z'λ[" ^ (number(length (idsetseq fid))) ^ "]";
fun	⦏λ_btype⦎ (itym:(string * TYPE) list) (t_ty:TYPE) (v_ty:TYPE) :TYPE =
		mk_→_type(
			list_mk_→_type(
				itymfunseq itym @
...
fun	⦏λ_bterm⦎ (itym:(string * TYPE) list) (t_ty:TYPE) (v_ty:TYPE) :TERM =
		mk_const (λ_bname (map fst itym), λ_btype itym t_ty v_ty);
=TEX
\subsubsection{definite description}


=SML
fun	⦏μ_bname⦎ (fid:string list) :string =
		"Z'μ[" ^ (number(length (idsetseq fid))) ^ "]";
fun	⦏μ_btype⦎ (itym:(string * TYPE) list) (ty:TYPE) :TYPE =
		mk_→_type(
			list_mk_→_type(
				itymfunseq itym @
				[g2_bind_type ty]
...
fun	⦏μ_bterm⦎ (itym:(string * TYPE) list) (ty:TYPE) :TERM =
		mk_const (μ_bname (map fst itym), μ_btype itym ty);
=TEX
...
fun	⦏schema_abs⦎ (itym:(string * TYPE) list) (ft:TERM) :TERM =
	...
	in
		list_mk_simple_λ (map mk_var (combine sl tyl),ft)
	end;
=TEX
