...
********************************************************************************
% usr011A.doc   ℤ $Date: 2004/01/19 12:44:00 $ $Revision: 1.15 $ $RCSfile: usr011A.doc,v $
...
Because of the above behaviour of the system in inferring types and actual generic parameters, a special global variable, which we have called `𝕌', turns out to be very useful.

`𝕌' may be though of as if defined by the abbreviation definition:

=GFT Z
𝕌[X] ≜ X
=TEX

If `𝕌' is used in some specification or expression without supplying an actual generic parameter, the type inferrer will infer an appropriate type, and will then use for the actual parameter the set of all elements of that type.
...
The second purpose is for expressing theorems which may be used for unconditional rewriting, since formulae universally quantified over `𝕌' can readily be specialised for any type-correct rewrite, whereas quantification over other expressions gives rise to proof obligations which must be discharged before specialisation for rewriting can take place.
...
The left hand operand of a cast must be an expression or schema expression rather than a predicate, and use of a cast (even if the right hand operand is simply `𝕌'), will therefore force interpretation of the left hand operand as an expression or schema expression (if this is possible).

The second feature is the operator $Π$.

$Π$ will accept as an operand only a predicate, and acts as an identity function on predicates.
...
=GFT Z_TERM
 ...
 |	⦏ZSchemaDec⦎	of TERM		(* schema expression *)
 	...
 	(* declaration list, e.g. ⓜdecl_of ⓩ[x,y:ℤ; File!]⌝⌝ *)
...
=SML
val dec1 = dec_of ⓩ[x,y:ℤ]⌝;
=GFT ProofPower output
val dec1 = ⓩⓜdec_ofⓩ[x, y : ℤ]⌝⌝⌝ : TERM
=TEX
...
=SML
val decl1 = decl_of ⓩ[x,y:ℤ; File!]⌝;
=GFT ProofPower output
val decl1 = ⓩⓜdecl_ofⓩ[x, y : ℤ; File!]⌝⌝⌝ : TERM
=TEX
...
=GFT ProofPower output
val pred2 = ⊢ ⓜdec_ofⓩ[x, y : ℤ]⌝⌝ ⇔ {x, y} ⊆ ℤ : THM
=TEX
...
=GFT ProofPower output
val pred4 = ⊢ ⓜdecl_ofⓩ[x, y : ℤ; File!]⌝⌝ ⇔ {x, y} ⊆ ℤ ∧ (File!) : THM
=TEX

Conversions taking predicates into declarations are also available:

=SML
val dec3 = z_pred_dec_conv ⓩ{x, y} ⊆ ℤ⌝;
=GFT ProofPower output
val dec3 = ⊢ {x, y} ⊆ ℤ ⇔ ⓜdec_ofⓩ[x, y : ℤ]⌝⌝ : THM
=TEX

=SML
val dec3 = z_pred_dec_conv ⓩΠ(File !)⌝;
=GFT ProofPower output
val dec3 = ⊢ (File!) ⇔ ⓜdec_ofⓩ[(File!)]⌝⌝ : THM
=TEX

Note that here the use of $Π$ was necessary in the term quotation to force $File!$ to be interpreted as a predicate rather than as a schema expression.
