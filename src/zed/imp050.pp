...
********************************************************************************
% imp050.doc ℤ 2004/01/19 12:44:00 1.45 imp050.doc,v
=TEX
...
Corrected incorrect mapping of $∀⋎s$.
...
\item[Issue 1.45 (2004/01/19)] The Z universal set is now called 𝕌.
...
=SML
fun ⦏int_to_tyvl⦎ (n : int) : TYPE list = (
	if n > 0
...
=TEX
=SML
fun ⦏int_to_sl⦎ (n : int) : string list = (
	if n > 0
...
=SML
fun ⦏int_to_itym⦎ (1 : int) : (string * TYPE) list =
	[("1", mk_z_var_type "1")]
|	int_to_itym n = (
...
=SML
fun ⦏string_to_itym⦎ (s : string) : (string * TYPE) list =
	int_to_itym(nat_of_string s);
=TEX
function to make a type list from a string
=SML
fun ⦏string_to_styl⦎ (s : string) : TYPE list =
...
=TEX
=SML
fun ⦏gen_z_semantic_type⦎ (ety : Unification.EXTYPE) : unit = (
	case ety of
...
=IGN
val tm = mk_z_seta (mk_z_decl [mk_z_dec([⌜a:ℤ⌝],ⓩℤ⌝)], mk_z_true, ⓩ[a,b,c:ℤ]⌝);
...
=IGN
val tm = mk_z_λ (mk_z_decl([mk_z_dec([⌜a:ℤ⌝],ⓩℤ⌝)]), mk_z_true, ⓩ[a,b,c:ℤ]⌝);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'λ",[[sn]], Nil) = dest_z_name nam;
=SML
fun	⦏λ_gen_semantic_const⦎ (tm : TERM) : THM = (
let	val nam = fst(dest_const (fst (strip_app tm)))
	handle Fail _ => term_fail "λ_gen_semantic_const" 50001 [tm];
in
	case dest_z_name nam of
	("Z'λ",[[sn]], Nil) => (get_cached_defn nam
		handle Fail _ => (
		let	val tyvl = string_to_styl sn
				handle (Fail _) =>
				term_fail "λ_gen_semantic_const" 50001 [tm];
			val tyv = mk_z_var_type "v";
			...
			val defn = mk_∀ (pack, body);
		in
			local_const_spec "λ_gen_semantic_const" (nam, cvar, defn)
		end))
	| _ => term_fail "λ_gen_semantic_const" 50001 [tm]
end);
=TEX

\subsection{Definite Description}
=IGN
val tm = mk_z_μ (mk_z_decl [mk_z_dec([⌜a:ℤ⌝],ⓩℤ⌝)], mk_z_true, ⓩ[a,b,c:ℤ]⌝);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'μ",[[sn]], Nil) = dest_z_name nam;
=SML
fun ⦏μ_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val nam = fst(dest_const(fst(strip_app tm)))
	handle Fail _ => term_fail "μ_gen_semantic_const" 50001 [tm];
in
	case dest_z_name nam of
	("Z'μ",[[sn]], Nil) => (get_cached_defn nam
		handle (Fail _) => (
		let	val tyvl = string_to_styl sn
				handle (Fail _) =>
				term_fail "μ_gen_semantic_const" 50001 [tm];
			val tyv = mk_z_var_type "v";
			val dpvty = mk_z_schema_type[("d",BOOL),("p",BOOL),("v",tyv)];
			val side = binding_type_spec dpvty;
			val (dpvcon,dpvprojs) = get_binding_info dpvty;
			...
			val defn = mk_∀ (pack, body);
		in
			local_const_spec "μ_gen_semantic_const" (nam, cvar, defn)
		end))
	| _ => term_fail "μ_gen_semantic_const" 50001 [tm]
end);
=TEX
...
=IGN
val tm = mk_z_∃⋎1 (mk_z_decl [mk_z_dec([⌜a:ℤ⌝],ⓩℤ⌝)], mk_z_true, mk_z_true);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'∃⋎1",[[sn]], Nil) = dest_z_name nam;
=SML
fun ⦏∃⋎1_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val nam = fst(dest_const(fst(strip_app tm)))
...
=IGN
val tm = mk_z_h_schema (mk_z_decl [mk_z_dec([⌜a:ℤ⌝, ⌜b:ℤ⌝],ⓩℤ⌝)], mk_z_true);
...
=IGN
val tm = mk_z_decor⋎s(ⓩ[x,y,xy:ℤ]⌝,"''");
val tm' = fst (strip_app tm);
val nam = (fst(dest_const tm'));
val ("Z'Decor⋎s",[cpl,decor], Nil) = dest_z_name nam;
=SML
fun ⦏decor⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
...
=TEX
=SML
fun ⦏pre⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val tm' = fst (strip_app tm);
	val nam = fst(dest_const tm')
...
=IGN
val tm = mk_z_hide⋎s (ⓩ[a,b,c:ℤ]⌝, ["c"]);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'Hide⋎s",[cpl1,cpl2], Nil) = dest_z_name nam;
=SML
fun ⦏hide⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val tm' = fst (strip_app tm);
	val nam = fst(dest_const tm')
...
val tm = mk_z_∧⋎s (ⓩ[a,b,c:ℤ]⌝, ⓩ[d:ℤ]⌝);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val (oper,[cpl1,cpl2], Nil) = dest_z_name nam;
=SML
fun ⦏dyop⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val tm'= fst (strip_app tm);
	val nam = fst(dest_const tm')
...
in
	...
	let	val (ra1ty,ra2ty,rresty) = (
		...
		val body = mk_eq(lhs,setcomp);
		val defn = list_mk_∀ ([sch1,sch2], body);
	in
...
=IGN
val tm = mk_z_↾⋎s (ⓩ[a,b,c:ℤ]⌝, ⓩ[b:ℤ]⌝);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'↾⋎s",[cpl1,cpl2], Nil) = dest_z_name nam;
=SML
fun ⦏↾⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val tm'= fst (strip_app tm);
	val nam = fst(dest_const tm')
...
=IGN
val tm = mk_z_Δ⋎s ⓩ[a,b,c:ℤ]⌝;
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'Δ⋎s",[cpl], Nil) = dest_z_name nam;
=SML
fun ⦏Δ⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val tm'= fst (strip_app tm);
	val nam = fst(dest_const tm')
	handle Fail _ => 	term_fail "Δ⋎s_gen_semantic_const" 50001 [tm];
in
	case dest_z_name nam of
	("Z'Δ⋎s",[cpl], Nil) => (get_cached_defn nam
	handle (Fail _) => (
	let	val (rargty,rresty) = (
			case strip_→_type(type_of tm') of
			[ty1,ty2] => (dest_z_power_type ty1,dest_z_power_type ty2)
			|_ => term_fail "Δ⋎s_gen_semantic_const" 50001 [tm]
			);
		val side = binding_type_spec rargty;
		val side = binding_type_spec rresty;
		...
		val defn = mk_∀(s,body);
	in
		local_const_spec "Δ⋎s_gen_semantic_const" (nam, cvar, defn)
	end))
	| _ => term_fail "Δ⋎s_gen_semantic_const" 50001 [tm]
end);
=TEX
...
=IGN
val tm = mk_z_Ξ⋎sⓩ[a,b,c:ℤ]⌝;
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'Ξ⋎s",[cpl], Nil) = dest_z_name nam;
=SML
fun ⦏Ξ⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val tm'= fst (strip_app tm);
	val nam = fst(dest_const tm')
	handle Fail _ => 	term_fail "Ξ⋎s_gen_semantic_const" 50001 [tm];
in
	case dest_z_name nam of
	("Z'Ξ⋎s",[cpl], Nil) => (get_cached_defn nam
	handle (Fail _) => (
	let	val (rargty,rresty) = (
			case strip_→_type(type_of tm') of
			[ty1,ty2] => (dest_z_power_type ty1,dest_z_power_type ty2)
			|_ => term_fail "Ξ⋎s_gen_semantic_const" 50001 [tm]
			);
		val side = binding_type_spec rargty;
		val side = binding_type_spec rresty;
		val (cona,_) = get_binding_info rargty;
		...
		val s = mk_var("sch",argty);
		val d = mk_z_decl[mk_z_schema_dec(s,""),mk_z_schema_dec(s,"'")];
		val p = mk_z_eq(mk_z_θ(s,""),mk_z_θ(s,"'"));
		val sch = mk_z_h_schema(d,p);
		val side = schema_gen_semantic_const sch;
		val lhs = mk_app(cvar, s);
		val body = mk_eq (lhs, sch);
		val defn = mk_∀(s, body);
	in
		local_const_spec "Ξ⋎s_gen_semantic_const" (nam, cvar, defn)
	end))
	| _ => term_fail "Ξ⋎s_gen_semantic_const" 50001 [tm]
end);
=TEX
...
=IGN
val tm = mk_z_∀⋎s (mk_z_decl [mk_z_dec([⌜a:ℤ⌝],ⓩℤ⌝)], mk_z_true, ⓩ[a,b,c:ℤ]⌝);
val tm = mk_z_∃⋎s (mk_z_decl [mk_z_dec([⌜a:ℤ⌝],ⓩℤ⌝)], mk_z_true, ⓩ[a,b,c:ℤ]⌝);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val (oper,[cpl1,cpl2], Nil) = dest_z_name nam;
=SML
fun ⦏quant⋎s_gen_semantic_const⦎ (tm:TERM) : THM = (
let	val tm'= fst (strip_app tm);
	val nam = fst(dest_const tm')
...
=IGN
val tm = mk_z_⨾⋎s (ⓩ[x,b,c,d':X]⌝, ⓩ[d:X;e:Y]⌝);
val tm = mk_z_⨾⋎s (ⓩ[qq',b,c,d':X]⌝, ⓩ[qq,d:X;e:Y]⌝);
val tm'= fst (strip_app tm);
val nam = fst(dest_const tm');
val ("Z'⨾⋎s",[cpl1,cpl2], Nil) = dest_z_name nam;
=SML
...
in
	...
	let	val (rargty1,rargty2,rresty) = (
		...
		val lhs = list_mk_app(cvar,[s1,s2]);
		val defn_tm = list_mk_∀([s1, s2], mk_eq(lhs,rhs));
	in
...
=SML
fun ⦏rename⋎s_gen_semantic_const⦎ (tm : TERM) : THM = (
...
=SML
	...
	val def_tm = mk_∀(argS, mk_eq(lhs,rhs));
in
