=IGN
********************************************************************************
dtd050.doc: this file is part of the PPZed system
...
********************************************************************************
% dtd050.doc ℤ 2002/10/17 16:26:32 1.27 dtd050.doc,v
=TEX
...
\def\Title{Detailed Design of Z Constant Generation}

\def\AbstractText{This document contains the detailed design of the definition of constants generated by the Z Abstract Syntax in the Z Proof Support System.}

\def\Reference{DS/FMU/IED/DTD050}
...
%% LaTeX2e port: \TPPtitle{Detailed Design of Z Constant Generation}  %% Mandatory field
%% LaTeX2e port: \def\TPPheadtitle{Detailed Design of Z Constant Generation}
%% LaTeX2e port: \TPPref{DS/FMU/IED/DTD050}  %% Mandatory field
%% LaTeX2e port: \def\SCCSversion{1.27 %
%% LaTeX2e port: }
...
%% LaTeX2e port: \TPPabstract{This document contains the detailed design of the definition of constants generated by the Z Abstract Syntax in the  Z Proof Support System.}
...
\begin{description}
\item [Issues 1.1 (1991/11/18) - 1.3 (1992/01/31)]
First draft versions.
\item [Issue 1.4 (1992/01/31)]
$get\_mode$ and $set\_mode$ replaced by a boolean flag.
\item [Issue 1.5 (1992/06/09)]Changed to new fonts.
\item [Issue 1.9 (1992/07/20) (20th July 1992)]
Added schema renaming definition mechanism.
Replaces uses of $\% down\%$.
\item [Issue 1.10 (1992/07/20) (20th July 1992)] Added support for binding display.
\item [Issue 1.18 (1992/08/24) (24 August 1992)] Provided a description of
the functionality and the design for Z generated semantic constants.
\item [Issue 1.19 (1992/10/15) (15th October 1992)]
Improved message 50001.
\item[Issue 1.20 (1992/11/20) (8th January 1993)]
Fixed error handling bug in $hide⋎s\_gen\_semantic\_const$.
\item[Issue 1.21 (1993/01/08) (8th January 1993)]
Added errors 50005, 50006.
\item[Issue 1.23 (1993/01/19) (19th January 1993)]
Added error 50007.
\item[Issue 1.24 (1994/05/07) (7th May 1994)]
...
\item[Issue 1.25 (1996/01/15) (12th January 1996)]
...
\item[Issue 1.28 (2004/07/29), 1.29 (2004/08/04)] Performance enhancements.
...
\item[Issue 1.31 (2010/04/01)] Support for empty schemas.
\item[2014/07/23]
...
=DOC
val ⦏get_tuple_info⦎ : TYPE -> TERM * TERM list;
=DESCRIBE
...
=DOC
val ⦏binding_type_spec⦎ : TYPE -> THM;
=DESCRIBE
This function take a binding type, e.g., of the form $ⓣ(ℤ, ℤ) \$"Z'S[a,b]⌝"$. It introduces the type into the current cache theory if required, and proceeds to formulate the constructor and the projections for instances of this type, e.g.,
=GFT Example
⊢ ⌜∀ t x1 x2
  ⦁ ⓩ(a ≜ x1, b ≜ x2).a⌝ = x1
      ∧ ⓩ(a ≜ x1, b ≜ x2).b⌝ = x2
...
=DOC
val ⦏tuple_type_spec⦎ : TYPE -> THM;
=DESCRIBE
This function take a tuple type, e.g., of the form $ⓣ(ℤ, ℤ) \$"Z'T[2]⌝"$. It introduces the type into the current cache theory if required, and proceeds to formulate the constructor and the projections for instances of this type, e.g.,
=GFT Example
⊢ ⌜∀ t x1 x2
  ⦁ ⓩ(x1, x2).1⌝ = x1 ∧ ⓩ(x1, x2).2⌝ = x2 ∧ ⓩ(t.1, t.2)⌝ = t⌝
=ENDDOC

=DOC
val ⦏gen_z_semantic_type⦎ : Unification.EXTYPE -> unit;
=DESCRIBE
This function ensures that the constructor of the tuple type or binding type given
as its argument is defined.
The type constructor is introduced into the current cache theory if
it is not already in scope.
The function does not recursively process the arguments of the type.
=ENDDOC
...
e.g., $Z'λ[2]$, $Z'Seta[6]$. The identifying
...
the constant. For example $Z'λ[2]$ represents a Z lambda
abstraction over two variables, e.g., $[X] (λ a b :X ⦁ X)$.
...
$Z'λ$&$λ\_gen\_semantic\_const$&$Z'λ[2]$ \\
$Z'×$&$×\_gen\_semantic\_const$&$Z'×[2]$ \\
$Z'μ$&$μ\_gen\_semantic\_const$&$Z'μ[2]$ \\
...
to the components of the schema. e.g., $⌜Z'Δ⋎s[a,b,c]⌝$
...
components. The type of $⌜Z'Δ⋎s[a,b,c]⌝$ contains
...
$Z'Δ⋎s$&$Δ⋎s\_gen\_semantic\_const$&$Z'Δ⋎s[a,b]$ \\
$Z'Ξ⋎s$&$Ξ⋎s\_gen\_semantic\_const$&$Z'Ξ⋎s[a,b]$ \\\hline
\end{tabular}
...
\begin{tabular}{|l|l|l|}\hline
Name&Description&Example \\\hline
$Z'Decor⋎s$&$decor⋎s\_gen\_semantic\_const$&$Z'Decor⋎s[a,b][']$ \\
$Z'∧⋎s$&$dyop⋎s\_gen\_semantic\_const$&$Z'∧⋎s[a,b][c,d]$ \\
$Z'∨⋎s$&$dyop⋎s\_gen\_semantic\_const$&$Z'∨⋎s[a,b][c,d]$ \\
$Z'⇒⋎s$&$dyop⋎s\_gen\_semantic\_const$&$Z'⇒⋎s[a,b][c,d]$ \\
$Z'⇔⋎s$&$dyop⋎s\_gen\_semantic\_const$&$Z'⇔⋎s[a,b][c,d]$ \\
$Z'↾⋎s$&$↾⋎s\_gen\_semantic\_const$&$Z'↾⋎s[a,b][a]$ \\
...
=DOC
val ⦏tuple_gen_semantic_const⦎ : TERM -> THM;
val ⦏seta_gen_semantic_const⦎ : TERM -> THM;
val ⦏λ_gen_semantic_const⦎ : TERM -> THM;
val ⦏×_gen_semantic_const⦎ : TERM -> THM;
val ⦏μ_gen_semantic_const⦎ : TERM -> THM;
val ⦏binding_gen_semantic_const⦎ : TERM -> THM;
val ⦏∃⋎1_gen_semantic_const⦎ : TERM -> THM;
val ⦏schema_gen_semantic_const⦎ : TERM -> THM;
val ⦏pre⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏Δ⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏Ξ⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏decor⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏dyop⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏↾⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏hide⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏quant⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏⨾⋎s_gen_semantic_const⦎ : TERM -> THM;
val ⦏rename⋎s_gen_semantic_const⦎ : TERM -> THM;
=DESCRIBE
...
val def1_9 = ⌜(λ pack ⦁ {x |∃ a1 a2 a3 a4 a5 a6 a7 a8 a9
...
val def1_10 = ⌜(λ pack ⦁ {x |∃ a1 a2 a3 a4 a5 a6 a7 a8 a9 a10
...
=GFT SML Input
val cs1_1 = ⌜∀ pack ⦁ seta1' pack = {x |∃ a1 ⦁ $"Z'S[d,p,v]d" (pack a1)
	∧ $"Z'S[d,p,v]p" (pack a1)
	∧ $"Z'S[d,p,v]v" (pack a1) = x}⌝;
val cs1_2 = ⌜∀ pack ⦁ seta2' pack = {x |∃ a1 a2
	⦁ $"Z'S[d,p,v]d" (pack a1 a2)
	∧ $"Z'S[d,p,v]p" (pack a1 a2)
	∧ $"Z'S[d,p,v]v" (pack a1 a2) = x}⌝;
val cs1_3 = ⌜∀ pack ⦁ seta3' pack = {x |∃ a1 a2 a3
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3) = x}⌝;
val cs1_4 = ⌜∀ pack ⦁ seta4' pack = {x |∃ a1 a2 a3 a4
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3 a4)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3 a4)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3 a4) = x}⌝;
val cs1_5 = ⌜∀ pack ⦁ seta5' pack = {x |∃ a1 a2 a3 a4 a5
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3 a4 a5)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3 a4 a5)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3 a4 a5) = x}⌝;
val cs1_6 = ⌜∀ pack ⦁ seta6' pack = {x |∃ a1 a2 a3 a4 a5 a6
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3 a4 a5 a6)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3 a4 a5 a6)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3 a4 a5 a6) = x}⌝;
val cs1_7 = ⌜∀ pack ⦁ seta7' pack = {x |∃ a1 a2 a3 a4 a5 a6 a7
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3 a4 a5 a6 a7)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3 a4 a5 a6 a7)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3 a4 a5 a6 a7) = x}⌝;
val cs1_8 = ⌜∀ pack ⦁ seta8' pack = {x |∃ a1 a2 a3 a4 a5 a6 a7 a8
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3 a4 a5 a6 a7 a8)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3 a4 a5 a6 a7 a8)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3 a4 a5 a6 a7 a8) = x}⌝;
val cs1_9 = ⌜∀ pack ⦁ seta9' pack = {x |∃ a1 a2 a3 a4 a5 a6 a7 a8 a9
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3 a4 a5 a6 a7 a8 a9)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3 a4 a5 a6 a7 a8 a9)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3 a4 a5 a6 a7 a8 a9) = x}⌝;
val cs1_10 = ⌜∀ pack ⦁ seta10' pack = {x |∃ a1 a2 a3 a4 a5 a6 a7 a8 a9 a10
	⦁ $"Z'S[d,p,v]d" (pack a1 a2 a3 a4 a5 a6 a7 a8 a9 a10)
	∧ $"Z'S[d,p,v]p" (pack a1 a2 a3 a4 a5 a6 a7 a8 a9 a10)
	∧ $"Z'S[d,p,v]v" (pack a1 a2 a3 a4 a5 a6 a7 a8 a9 a10) = x}⌝;
